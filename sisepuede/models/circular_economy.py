...
class CircularEconomy:
    ...
    def check_df_fields(
        ...
        if not set(check_fields).issubset(df_ce_trajectories.columns):
            ...
            raise KeyError(
                f"Circular Economy projection cannot proceed: The fields {set_missing} are missing."
            )

    def get_wali_dict_trww_categories_to_wali_fraction_variables(
        self,
    ) -> Dict:
    ...
    def _initialize_input_output_components(
        ...
        * self.required_variables_wali
        ...
        required_vars_wali, output_vars_wali = (
            self.model_attributes.get_input_output_fields(
                [
                    x
                    for x in subsectors
                    if (x != self.model_attributes.subsec_name_waso)
                ],
            )
        )

        # get input/output model variables`
        required_model_vars = sorted(
            list(
                set(
                    [
    ...
    def _initialize_other_properties(
        ...
        * self.vars_wali_to_trww
        ...
        vars_wali_to_trww = (
            self.model_attributes.get_ordered_vars_by_nonprimary_category(
                self.model_attributes.subsec_name_wali,
                self.model_attributes.subsec_name_trww,
            )
        )
    ...
    def _initialize_parameters_biophysical(
        self,
    ) -> None:
    ...
    def _initialize_subsector_vars_trww(
        self,
    ) -> None:
        ...
        * self.modvar_trww_****
        ...
        self.modvar_trww_ef_n2o_wastewater_treatment = (
            ":math:\\text{N}_2\\text{O} Wastewater Treatment Emission Factor"
        )
    ...
    def _initialize_subsector_vars_wali(
        self,
    ) -> None:
        """Initialize model variables, categories, and indices associated with
        ...
        * self.modvar_wali_****
        ...
        self.modvar_wali_max_bod_capac = (
            "Maximum BOD :math:\\text{CH}_4 Producing Capacity"
        )
        self.modvar_wali_max_cod_capac = (
            "Maximum COD :math:\\text{CH}_4 Producing Capacity"
        )
        ...
        self.modvar_wali_treatpath_advanced_aerobic = (
            "Treatment Fraction Advanced Aerobic"
        )
        self.modvar_wali_treatpath_advanced_anaerobic = (
            "Treatment Fraction Advanced Anaerobic"
        )
        self.modvar_wali_treatpath_septic = "Treatment Fraction Septic"
        self.modvar_wali_treatpath_latrine_improved = (
            "Treatment Fraction Improved Latrine"
        )
        self.modvar_wali_treatpath_latrine_unimproved = (
            "Treatment Fraction Unimproved Latrine"
        )
        self.modvar_wali_treatpath_secondary_aerobic = (
            "Treatment Fraction Secondary Aerobic"
        )
        self.modvar_wali_treatpath_secondary_anaerobic = (
            "Treatment Fraction Secondary Anaerobic"
        )
        self.modvar_wali_treatpath_untreated_no_sewerage = (
            "Treatment Fraction Untreated No Sewerage"
        )
        self.modvar_wali_treatpath_untreated_with_sewerage = (
    ...
    def _initialize_subsector_vars_waso(
        self,
    ) -> None:
        """Initialize model variables, categories, and indices associated with
        ...
        * self.modvar_waso_****
        """
        self.modvar_waso_annual_vkmt_per_collection_vehicle = (
            "Average VKMT Per Waste Collection Vehicle"
        )
        self.modvar_waso_annual_waste_collected_per_collection_vehicle = (
        ...
        self.modvar_waso_ef_ch4_biogas = (
            ":math:\\text{CH}_4 Anaerobic Biogas Emission Factor"
        )
        self.modvar_waso_ef_ch4_compost = (
            ":math:\\text{CH}_4 Composting Emission Factor"
        )
        self.modvar_waso_ef_ch4_incineration_isw = (
            ":math:\\text{CH}_4 ISW Incineration Emission Factor"
        )
        self.modvar_waso_ef_ch4_incineration_msw = (
            ":math:\\text{CH}_4 MSW Incineration Emission Factor"
        )
        self.modvar_waso_ef_n2o_compost = (
            ":math:\\text{N}_2\\text{O} Composting Emission Factor"
        )
        self.modvar_waso_ef_n2o_incineration = (
            ":math:\\text{N}_2\\text{O} Incineration Emission Factor"
        )
        ...
        self.modvar_waso_emissions_ch4_biogas = (
            ":math:\\text{CH}_4 Emissions from Anaerobic Biogas"
        )
        self.modvar_waso_emissions_ch4_compost = (
            ":math:\\text{CH}_4 Emissions from Composting"
        )
        self.modvar_waso_emissions_ch4_incineration = (
            ":math:\\text{CH}_4 Emissions from Incineration"
        )
        self.modvar_waso_emissions_ch4_landfill = (
            ":math:\\text{CH}_4 Emissions from Landfills"
        )
        self.modvar_waso_emissions_ch4_open_dump = (
            ":math:\\text{CH}_4 Emissions from Open Dumping"
        )
        self.modvar_waso_emissions_co2_incineration = (
            ":math:\\text{CO}_2 Emissions from Incineration"
        )
        self.modvar_waso_emissions_n2o_compost = (
            ":math:\\text{N}_2\\text{O} Emissions from Composting"
        )
        self.modvar_waso_emissions_n2o_incineration = (
            ":math:\\text{N}_2\\text{O} Emissions from Incineration"
        )
        ...
        self.modvar_waso_rf_landfill_gas_to_ch4 = (
            ":math:\\text{CH}_4 Recovery Factor Landfill Gas"
        )
    ...
    def fod(
        ...
        if len(array_waso_waste.shape) == 1:
            ...
            raise FODError(
                "array_waso_waste should be a two dimensional array (rows are time periods, columns are categories)"
            )

        if len(array_k.shape) == 1:
            if len(array_k) != array_waso_waste.shape[1]:
                raise FODError(
                    "array_k does not have the same number of categories as array_waso_waste."
                )

        elif len(array_k.shape) == 2:
            if array_k.shape != array_waso_waste.shape:
                raise FODError(
                    f"incompatible array specification of array_k (shape '{array_k.shape}'). It should have shape '{array_waso_waste.shape}'"
                )

        elif len(vec_ddocm_factors) != array_waso_waste.shape[1]:
            raise FODError(
                "vec_ddocm_factors does not have the same number of categories as array_waso_waste."
            )

        elif len(vec_mcf) != array_waso_waste.shape[0]:
            raise FODError(
                "vec_mcf does not have the same time periods as array_waso_waste."
            )

        elif (type(vec_oxf) == np.ndarray) & (
            len(vec_oxf) != array_waso_waste.shape[0]
        ):
            raise FODError(
                "vec_oxf does not have the same time periods as array_waso_waste."
            )

        elif (type(vec_frac_captured) == np.ndarray) & (
            len(vec_frac_captured) != array_waso_waste.shape[0]
        ):
            raise FODError(
                "vec_frac_captured does not have the same time periods as array_waso_waste."
            )
        ...
        np.put(
            array_ddocm_accumulated,
            np.arange(0, len(array_ddocm_accumulated[0])),
            array_waso_waste[0],
        )
        ...
        for i in range(1, len(array_waso_waste)):
            ...
            vec_ddocm_accumulated_cur = (
                array_waso_waste[i] + array_ddocm_accumulated[i - 1] * vec_k
            )
        ...
        array_ch4_total = (
            array_ddocm_decomposed
            * self.factor_molecular_weight_ch4
            * self.landfill_gas_frac_methane
        )
        array_ch4_captured = (
            array_ch4_total.transpose() * vec_frac_captured
        ).transpose()
    ...
    def get_waso_historical_solid_waste(
        self,
        ...
        **CAUTION**: Historical is currently undefined.
        ...
        if (
            method
            not in self.model_attributes.configuration.valid_historical_solid_waste_method
        ):
            valid_vals = sf.format_print_list(
                self.model_attributes.configuration.valid_historical_solid_waste_method
            )
            raise ValueError(
                f"Invalid specification of historical waste retrieval method '{method}': Method not found. Valid values are {valid_vals}."
            )
        ...
        if method == "historical":
            ...
            method = "back_project"
            self._log(
                f"Updating method to '{method}' while historical is in development.",
                type_log="warning",
            )
        ...
        if method == "back_project":
            ...
            if type(n_periods) != int:
                raise ValueError(
                    "Error: specify a number of periods to project solid waste backwards."
                )
    ...
    def get_waso_integrated_waste_passthroughs(
        self,
    ...
    def project_protein_consumption(
        self,
        df_ce_trajectories: pd.DataFrame,
        vec_pop: np.ndarray,
        ...
        vec_wali_frac_protein_in_diet_with_rm = (
        ...
        vec_wali_frac_protein_in_diet_without_rm = (
            self.model_attributes.extract_model_variable(
                df_ce_trajectories,
                self.modvar_wali_frac_protein_without_red_meat,
                override_vector_for_single_mv_q=True,
        ...
        vec_wali_protein_scalar_no_rm = (
            vec_wali_frac_protein_in_diet_without_rm
            / vec_wali_frac_protein_in_diet_with_rm
        )
        vec_wali_protein_scalar = (
            vec_gnrl_frac_eating_red_meat
            + vec_wali_protein_scalar_no_rm * (1 - vec_gnrl_frac_eating_red_meat)
        ).flatten()
        ...
        vec_wali_protein_per_capita *= self.model_attributes.configuration.get(
            "days_per_year"
        )
        ...
        if modvar_proj_protein_driver == self.modvar_lvst_demand_domestic:
            ...
            vec_protein_growth = np.concatenate(
                [
                    np.ones(1),
                    np.cumprod(vec_protein_growth[1:] / vec_protein_growth[0:-1]),
                ]
            )
    ...
    def project_waste_liquid(
        ...
        * The df_ce_trajectories.project_waste_liquid method will run on valid
        ...
        if df_se_internal_shared_variables is None:
            df_ce_trajectories, df_se_internal_shared_variables = (
                self.model_socioeconomic.project(df_ce_trajectories)
            )
        ...
        if (
            (dict_dims is None)
            | (n_projection_time_periods is None)
            | (projection_time_periods is None)
        ):
            (
                dict_dims,
                df_ce_trajectories,
        ...
        vec_wali_bod_percap_init *= self.model_attributes.configuration.get(
            "days_per_year"
        )
        ...
        array_wali_vol_domww_percap = (
            array_wali_vol_domww_percap.transpose() * vec_wali_bod_correction
        ).transpose()
        ...
        array_wali_bod_percap = (
            array_wali_bod_percap.transpose() * vec_wali_scale_percapita_dem
        ).transpose()
        array_wali_vol_domww_percap = (
            array_wali_vol_domww_percap.transpose() * vec_wali_scale_percapita_dem
        ).transpose()

        # total bod (kg), cod (tonne), and wastewater (m3) generated
        array_wali_bod_total = (
            array_wali_bod_percap.transpose() * array_pop.transpose()
        ).transpose()
        array_wali_domww_total = (
            array_wali_vol_domww_percap.transpose() * array_pop.transpose()
        ).transpose()
        array_wali_cod_total = (array_wali_cod_pergdp.transpose() * vec_gdp).transpose()
        array_wali_indww_total = (
            array_wali_vol_indww_per_gdp.transpose() * vec_gdp
        ).transpose()
        ...
        array_trww_total_bod_by_pathway = np.zeros(
            (len(attr_trww.key_values), n_projection_time_periods)
        )
        ...
        factor_trww_emissions_mass_to_tow_mass = (
            self.model_attributes.get_mass_equivalent(
        ...
        array_trww_total_ww_bod_by_pathway = (
            array_trww_total_ww_bod_by_pathway.transpose()
        )
        array_trww_total_ww_cod_by_pathway = (
            array_trww_total_ww_cod_by_pathway.transpose()
        )
        array_trww_total_ww_by_pathway = (
            array_trww_total_ww_bod_by_pathway + array_trww_total_ww_cod_by_pathway
        )
        ...
        array_mask_sludge = np.sign(array_trww_krem) + np.sign(
            array_trww_septic_compliance
        )
        ...
        array_trww_tow_bod_removed_sludge = (
            (array_trww_frac_tow_removed + array_trww_septic_compliance * 0.5)
            * array_trww_total_bod_by_pathway
            * array_mask_sludge
        )
        array_trww_tow_bod_not_removed = (
            array_trww_total_bod_by_pathway - array_trww_tow_bod_removed_sludge
        )
        array_trww_tow_cod_removed_sludge = (
            (array_trww_frac_tow_removed + array_trww_septic_compliance * 0.5)
            * array_trww_total_cod_by_pathway
            * array_mask_sludge
        )
        array_trww_tow_cod_not_removed = (
            array_trww_total_cod_by_pathway - array_trww_tow_cod_removed_sludge
        )

        # apply methane correction factor to estimate methane emissions (these emissions are in co2e)
        array_trww_emissions_ch4_bod = (
            (array_trww_tow_bod_not_removed * array_trww_mcf).transpose()
            * vec_wali_bod_max_bo
        ).transpose()
        array_trww_emissions_ch4_cod = (
            (array_trww_tow_cod_not_removed * array_trww_mcf).transpose()
            * vec_wali_cod_max_bo
        ).transpose()
        array_trww_bod_equivalent_removed_sludge = (
            array_trww_tow_bod_removed_sludge
            + (
                array_trww_tow_cod_removed_sludge.transpose()
                * (vec_wali_cod_max_bo / vec_wali_bod_max_bo)
            ).transpose()
        )
        array_trww_emissions_ch4_treatment = (
            array_trww_emissions_ch4_bod + array_trww_emissions_ch4_cod
        )
        ...
        array_trww_ch4_recovered = (
            array_trww_frac_ch4_recovered * array_trww_emissions_ch4_treatment
        )
        ...
        array_trww_mass_removed_sludge = (
            array_trww_bod_equivalent_removed_sludge / array_trww_krem
        ) * factor_trww_emissions_mass_to_tow_mass
        ...
        array_trww_total_nitrogen_dom = (
            array_trww_total_bod_by_pathway.transpose()
            / np.sum(array_trww_total_bod_by_pathway, axis=1)
        )
        array_trww_total_nitrogen_dom = (
            array_trww_total_nitrogen_dom * vec_wali_total_nitrogen_dom
        ).transpose()
        array_trww_total_phosphorous_dom = (
            array_trww_total_bod_by_pathway.transpose()
            * vec_wali_phosphorous_density_dom
        ).transpose()
        ...
        scalar_wali_cod_mass_to_bod_mass = (
            self.model_attributes.get_variable_unit_conversion_factor(
                self.modvar_wali_nitrogen_density_ww_ind,
                self.modvar_wali_protein_per_capita,
                "mass",
        ...
        array_trww_total_nitrogen_ind = (
            array_trww_total_cod_by_pathway.transpose()
            / np.sum(array_trww_total_cod_by_pathway, axis=1)
        )
        array_trww_total_phosphorous_ind = (
            array_trww_total_nitrogen_ind * vec_wali_phosphorous_density_ind
        ).transpose() * array_trww_total_ww_cod_by_pathway
        array_trww_total_nitrogen_ind = (
            array_trww_total_nitrogen_ind * vec_wali_nitrogen_density_ind
        ).transpose() * array_trww_total_ww_cod_by_pathway
        ...
        array_trww_total_nitrogen = (
            array_trww_total_nitrogen_dom
            + array_trww_total_nitrogen_ind * scalar_wali_cod_mass_to_bod_mass
        )
        array_trww_total_phosphorous = (
            array_trww_total_phosphorous_dom
            + array_trww_total_phosphorous_ind * scalar_wali_cod_mass_to_bod_mass
        )
        ...
        array_trww_total_n_effluent = array_trww_total_nitrogen * (
            1 - array_trww_frac_n_removed
        )
        array_trww_total_p_effluent = array_trww_total_phosphorous * (
            1 - array_trww_frac_p_removed
        )
        ...
        array_trww_emissions_n2o_effluent = (
            array_trww_emissions_n2o_effluent.transpose()
        )

        # calculate pollutants (N & P) -- get scalars for output units
        scalar_trww_bod_mass_to_n_removed = (
            self.model_attributes.get_variable_unit_conversion_factor(
                self.modvar_wali_protein_per_capita,
        ...
        scalar_trww_bod_mass_to_p_removed = (
            self.model_attributes.get_variable_unit_conversion_factor(
                self.modvar_wali_protein_per_capita,
        ...
        scalar_trww_bod_mass_to_n_effluent = (
            self.model_attributes.get_variable_unit_conversion_factor(
                self.modvar_wali_protein_per_capita,
        ...
        scalar_trww_bod_mass_to_p_effluent = (
            self.model_attributes.get_variable_unit_conversion_factor(
                self.modvar_wali_protein_per_capita,
    ...
    def project_waste_solid(
        ...
        * The df_ce_trajectories.project_waste_liquid method will run on valid
        ...
        if type(None) in [
            type(dict_dims),
            type(n_projection_time_periods),
            type(projection_time_periods),
        ]:
            (
                dict_dims,
                df_ce_trajectories,
        ...
        array_waso_waste_biogas = (
            dict_waso_comp_biogas_check[self.modvar_waso_frac_biogas]
            * array_waso_total_by_category
        )
        array_waso_waste_compost = (
            dict_waso_comp_biogas_check[self.modvar_waso_frac_compost]
            * array_waso_total_by_category
        )
        array_waso_total_by_category -= (
            array_waso_waste_biogas + array_waso_waste_compost
        )
        ...
        array_waso_emissions_ch4_biogas *= (
            array_waso_ef_ch4_biogas * factor_waso_mass_to_emission_mass
        )
        vec_biogas_recovered = (
            np.sum(array_waso_emissions_ch4_biogas, axis=1)
            * vec_waso_biogas_recovery_factor
        )

        # adjust emissions from biogas down to account for recovery; then, rescale recovery to output units
        array_waso_emissions_ch4_biogas = (
            array_waso_emissions_ch4_biogas.transpose()
            * (1 - vec_waso_biogas_recovery_factor)
        ).transpose()
        ...
        array_waso_emissions_ch4_compost *= (
            (array_waso_ef_ch4_compost * factor_waso_mass_to_emission_mass).transpose()
            * (1 - vec_waso_ch4_flared_compost)
        ).transpose()
        ...
        array_waso_emissions_n2o_compost *= (
            array_waso_ef_n2o_compost * factor_waso_mass_to_emission_mass
        )
        ...
        vec_waso_cat_attr_dry_matter_content_as_fraction_wet_weight = (
        ...
        vec_waso_cat_attr_doc_content_as_fraction_wet_waste = (
            self.model_attributes.get_ordered_category_attribute(
                self.subsec_name_waso,
        ...
        vec_waso_cat_attr_doc_content_as_fraction_dry_waste = (
            self.model_attributes.get_ordered_category_attribute(
                self.subsec_name_waso,
        ...
        vec_waso_cat_attr_docf_degradable = (
            self.model_attributes.get_ordered_category_attribute(
                self.subsec_name_waso,
                "docf_degradable",
                return_type=np.ndarray,
        ...
        vec_waso_cat_attr_total_carbon_content_as_fraction_dry_weight = (
            self.model_attributes.get_ordered_category_attribute(
                self.subsec_name_waso,
        ...
        vec_waso_cat_attr_fossil_carbon_fraction_as_fraction_total_carbon = (
            self.model_attributes.get_ordered_category_attribute(
                self.subsec_name_waso,
        ...
        array_waso_waste_incineration = (
            dict_waso_check_non_recycle_pathways[
                self.modvar_waso_frac_nonrecycled_incineration
            ].flatten()
            * array_waso_total_by_category.transpose()
        ).transpose()
        ...
        vec_waso_frac_isw_incinerated_for_energy = (
            self.model_attributes.extract_model_variable(
                df_ce_trajectories,
                self.modvar_waso_frac_recovered_for_energy_incineration_isw,
                return_type="array_base",
        ...
        vec_waso_ef_incineration_co2 = (
            vec_waso_cat_attr_dry_matter_content_as_fraction_wet_weight
            * vec_waso_cat_attr_total_carbon_content_as_fraction_dry_weight
            * vec_waso_cat_attr_fossil_carbon_fraction_as_fraction_total_carbon
            * self.factor_c_to_co2
        )
        array_waso_emissions_co2_incineration = np.sum(
            array_waso_waste_incineration
            * vec_waso_ef_incineration_co2
            * factor_waso_mass_to_emission_mass,
            axis=1,
        )
        ...
        vec_waso_emissions_ch4_incineration_isw = (
            vec_waso_ef_ch4_incineration_isw
            * vec_waso_isw_total
            * factor_waso_mass_to_emission_mass
        )
        vec_waso_emissions_ch4_incineration_msw = (
            vec_waso_ef_ch4_incineration_msw
            * vec_waso_msw_total
            * factor_waso_mass_to_emission_mass
        )
        ...
        n_periods_bp = self.model_attributes.configuration.get(
        ...
        array_waso_waste_landfill = array_waso_waste_landfill[
            rowind_waso_model_periods_landfill
        ]
        array_waso_emissions_ch4_landfill = array_waso_emissions_ch4_landfill[
            rowind_waso_model_periods_landfill
        ]
        vec_waso_landfill_gas_recovered = vec_waso_landfill_gas_recovered[
            rowind_waso_model_periods_landfill
        ]
        ...
        array_waso_waste_open_dump = array_waso_waste_open_dump[
            rowind_waso_model_periods_open_dump
        ]
        array_waso_emissions_ch4_open_dump *= factor_waso_mass_to_emission_mass
        array_waso_emissions_ch4_open_dump = array_waso_emissions_ch4_open_dump[
            rowind_waso_model_periods_open_dump
        ]
    ...
    def project(
        ...
        * The df_ce_trajectories.project method will run on valid time periods
        ...
        # project solid waste
        df_in = (
            pd.concat([df_ce_trajectories, df_waso_sludge[1]], axis=1)
            if df_waso_sludge
            else df_ce_trajectories
        )
        df_out += [
