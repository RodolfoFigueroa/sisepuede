...
class EnergyProduction:
    ...
    def check_df_fields(
        ...
        if subsector == "All":
            ...
            if var_type == "input":
                ...
                ignore_fields, check_fields = (
                    self.model_attributes.get_input_output_fields([subsector])
                )
            else:
                raise ValueError(
    ...
    def get_enfu_dict_subsectors_to_energy_variables(
        self,
    ) -> Dict:
    ...
    def get_entc_dict_subsectors_to_emission_variables(
        self,
    ) -> Dict:
        """Return a dictionary with emission-producing energy subsectorz as keys
    ...
    def _initialize_dict_tables_required_to_required_fields(
        self,
    ) -> None:
    ...
    def _initialize_input_output_components(
        ...
        * self.required_variables
        ...
        ]  # self.model_attributes.get_setor_subsectors("Energy")
    ...
    def _initialize_julia_environment(
        self,
    ) -> None:
    ...
    def _initialize_julia(
        ...
        if not os.path.exists(dir_jl):
            ...
            msg = f"Path to Julia '{dir_jl}' not found. The fuel production model cannot be run."
        ...
        ##  SET THE PACKAGE REQUIREMENTS AND ENVIRONMENT

        self._initialize_julia_environment()

        ##  LOAD SOME PACKAGES

        self._log("Importing Julia...", type_log="info")
        from juliacall import Main as julia_main

        self.julia_main = julia_main

        # import into Main required packages
        self.julia_main.seval("using NemoMod")
        self.julia_main.seval("using JuMP")
        ...
        solver = (
            self.model_attributes.configuration.get("nemomod_solver")
            if (solver is None)
            else solver
        )
        ...
        # load
        try:
            self.julia_main.seval(f"using {self.solver_package}")
            self.solver_module = getattr(self.julia_main, self.solver_package)

            self._log(
                f"Successfully initialized JuMP optimizer from solver module {self.solver_package}.",
                type_log="info",
            )

        except Exception as e:
            self._log(
                f"An error occured while trying to initialize the JuMP optimizer from package: {e}",
                type_log="error",
            )
    ...
    def _initialize_nemomod_fields(
        self,
    ) -> None:
    ...
    def _initialize_nemomod_output_tables(
        self,
    ) -> None:
    ...
    def _initialize_nemomod_reference_dict(
        self,
        nemomod_reference_files: Union[str, dict],
        ...
        for k in dict_out:
            ...
            if self.field_nemomod_region in dict_out[k]:
                df_filt = dict_out[k][
                    dict_out[k][self.field_nemomod_region].isin(attr_region.key_values)
                ]
                if len(df_filt) == 0:
                    raise RuntimeError(
                ...
                if filter_regions_to_config:
                    ...
                    sf.check_set_values(
                        regions_config,
                        df_filt[self.field_nemomod_region],
                    )
                    df_filt = df_filt[
                    ...
                    if not check_regions:
                        missing_vals = sf.print_setdiff(
                            set(regions_config),
                            set(df_filt[self.field_nemomod_region]),
                        )
                        raise RuntimeError(
        ...
        sf.check_set_values(
            [
                ...
                if (x not in [self.field_nemomod_region, self.field_nemomod_time_slice])
            ],
    ...
    def _initialize_subsector_vars_enfu(
        self,
    ) -> None:
        ...
        * self.modvar_enfu_****
        ...
        self.modvar_enfu_ef_combustion_co2 = (
            ":math:\\text{CO}_2 Combustion Emission Factor"
        )
        self.modvar_enfu_ef_combustion_mobile_ch4 = (
            ":math:\\text{CH}_4 Mobile Combustion Emission Factor"
        )
        self.modvar_enfu_ef_combustion_mobile_n2o = (
            ":math:\\text{N}_2\\text{O} Mobile Combustion Emission Factor"
        )
        ...
        self.modvar_enfu_production_frac_petroleum_refinement = (
            "Petroleum Refinery Production Fraction"
        )
        self.modvar_enfu_production_frac_natural_gas_processing = (
            "Natural Gas Processing Fraction"
        )
    ...
    def _initialize_subsector_vars_entc(
        self,
    ) -> None:
        """Initialize model variables, categories, and indicies associated with
        ...
        * self.modvar_entc_****
        ...
        self.modvar_entc_ef_scalar_ch4 = (
            ":math:\\text{CH}_4 NemoMod EmissionsActivityRatio Scalar"
        )
        self.modvar_entc_ef_scalar_co2 = (
            ":math:\\text{CO}_2 NemoMod EmissionsActivityRatio Scalar"
        )
        self.modvar_entc_ef_scalar_n2o = (
            ":math:\\text{N}_2\\text{O} NemoMod EmissionsActivityRatio Scalar"
        )
        ...
        self.modvar_entc_fuelprod_emissions_activity_ratio_ch4 = (
            ":math:\\text{CH}_4 Fuel Production NemoMod EmissionsActivityRatio"
        )
        self.modvar_entc_fuelprod_emissions_activity_ratio_co2 = (
            ":math:\\text{CO}_2 Fuel Production NemoMod EmissionsActivityRatio"
        )
        self.modvar_entc_fuelprod_emissions_activity_ratio_n2o = (
            ":math:\\text{N}_2\\text{O} Fuel Production NemoMod EmissionsActivityRatio"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_coal_deposits = (
            "Fuel Production NemoMod InputActivityRatio Coal Deposits"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_crude = (
            "Fuel Production NemoMod InputActivityRatio Crude"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_diesel = (
            "Fuel Production NemoMod InputActivityRatio Diesel"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_electricity = (
            "Fuel Production NemoMod InputActivityRatio Electricity"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_gasoline = (
            "Fuel Production NemoMod InputActivityRatio Gasoline"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_hydrogen = (
            "Fuel Production NemoMod InputActivityRatio Hydrogen"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_natural_gas = (
            "Fuel Production NemoMod InputActivityRatio Natural Gas"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_natural_gas_unprocessed = (
            "Fuel Production NemoMod InputActivityRatio Natural Gas Unprocessed"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_oil = (
            "Fuel Production NemoMod InputActivityRatio Oil"
        )
        self.modvar_entc_fuelprod_input_activity_ratio_water = (
            "Fuel Production NemoMod InputActivityRatio Water"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_coal = (
            "Fuel Production NemoMod OutputActivityRatio Coal"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_diesel = (
            "Fuel Production NemoMod OutputActivityRatio Diesel"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_gasoline = (
            "Fuel Production NemoMod OutputActivityRatio Gasoline"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_hgl = (
            "Fuel Production NemoMod OutputActivityRatio Hydrocarbon Gas Liquids"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_hydrogen = (
            "Fuel Production NemoMod OutputActivityRatio Hydrogen"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_kerosene = (
            "Fuel Production NemoMod OutputActivityRatio Kerosene"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_natural_gas = (
            "Fuel Production NemoMod OutputActivityRatio Natural Gas"
        )
        self.modvar_entc_fuelprod_output_activity_ratio_oil = (
        ...
        self.modvar_entc_nemomod_emissions_ch4_elec = (
            "NemoMod :math:\\text{CH}_4 Emissions from Electricity Generation"
        )
        self.modvar_entc_nemomod_emissions_co2_elec = (
            "NemoMod :math:\\text{CO}_2 Emissions from Electricity Generation"
        )
        self.modvar_entc_nemomod_emissions_n2o_elec = (
            "NemoMod :math:\\text{N}_2\\text{O} Emissions from Electricity Generation"
        )
        self.modvar_entc_nemomod_emissions_ch4_fpr = (
            "NemoMod :math:\\text{CH}_4 Emissions from Fuel Processing and Refinement"
        )
        self.modvar_entc_nemomod_emissions_co2_fpr = (
            "NemoMod :math:\\text{CO}_2 Emissions from Fuel Processing and Refinement"
        )
        ...
        self.modvar_entc_nemomod_emissions_ch4_mne = (
            "NemoMod :math:\\text{CH}_4 Emissions from Fuel Mining and Extraction"
        )
        self.modvar_entc_nemomod_emissions_co2_mne = (
            "NemoMod :math:\\text{CO}_2 Emissions from Fuel Mining and Extraction"
        )
        ...
        self.modvar_entc_nemomod_emissions_subsector_ccsq_co2 = (
            "NemoMod :math:\\text{CO}_2 Emissions from Electricity Generation for CCSQ"
        )
        ...
        self.modvar_entc_nemomod_emissions_subsector_scoe_co2 = (
            "NemoMod :math:\\text{CO}_2 Emissions from Electricity Generation for SCOE"
        )
        ...
        self.modvar_entc_nemomod_emissions_subsector_ccsq_ch4 = (
            "NemoMod :math:\\text{CH}_4 Emissions from Electricity Generation for CCSQ"
        )
        ...
        self.modvar_entc_nemomod_emissions_subsector_scoe_ch4 = (
            "NemoMod :math:\\text{CH}_4 Emissions from Electricity Generation for SCOE"
        )
    ...
    def _initialize_subsector_vars_enst(
        self,
    ) -> None:
        """Initialize model variables, categories, and indicies associated with
    ...
    def _set_dict_enfu_fuel_categories_to_entc_variables(
        self,
    ...
    def transform_field_year_nemomod(
        self,
        vector: list,
        ...
        if time_period_as_year:
            vector_out = (
                (vector_out + shift)
                if (direction == "to_nemomod")
                else (vector_out - shift)
            )
    ...
    def add_index_field_from_key_values(
        self,
        ...
        if field_index not in df_input.columns:
            if len(df_input) == 0:
                ...
                df_input = df_input[
                    [field_index] + [x for x in order_cols if (x != field_dummy)]
                ]
            # check shape
            elif (len(df_input) == len(index_values)) or (
                not (
                    isinstance(index_values, list)
                    or isinstance(index_values, np.ndarray)
                )
            ):
    ...
    def add_index_field_region(
        ...
        # get regions
        field_region = (
            self.field_nemomod_region if (field_region is None) else field_region
        )
    ...
    def add_index_field_technology(
        self,
        df_input: pd.DataFrame,
        ...
        field_technology = (
            self.field_nemomod_technology
            if (field_technology is None)
            else field_technology
        )
        ...
        techs = (
            ...
            if (restriction_technologies is not None)
            else techs
        )
        # add to output using outer product
        df_input = self.add_index_field_from_key_values(
    ...
    def add_multifields_from_key_values(
        self,
        ...
        if len([x for x in fields_to_add if (x != self.field_nemomod_id)]) > 0:
            (
                df_input.rename(
                    columns={self.field_nemomod_id: field_id_rnm},
                    inplace=True,
                )
        ...
        fields_sort_hierarchy = [
            x
            for x in self.fields_nemomod_sort_hierarchy
            if (x in fields_to_add) and (x != self.field_nemomod_id)
        ]
        fields_sort_hierarchy = (
            fields_sort_hierarchy + [field_id_rnm]
            if (field_id_rnm in df_input.columns)
            else fields_sort_hierarchy
        )
        df_input = (
            df_input.sort_values(by=fields_sort_hierarchy).reset_index(drop=True)
            if (len(fields_sort_hierarchy) > 0)
            else df_input
        )
        df_input.drop([field_id_rnm], axis=1, inplace=True) if (
            field_id_rnm in df_input.columns
        ) else None

        # add the final id field if necessary
        df_input = (
            self.add_index_field_id(df_input)
            if (self.field_nemomod_id in fields_to_add)
            else df_input
        )
        df_input = df_input[
            [x for x in self.fields_nemomod_sort_hierarchy if x in df_input.columns]
        ]

        return df_input

    def allocate_entc_emissions_by_energy_demand(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        vec_enfu_frac_from_exports = np.nan_to_num(
            vec_enfu_exports / vec_enfu_total_production,
            nan=0.0,
            posinf=0.0,
        )
        vec_enfu_frac_from_others = 1 - vec_enfu_frac_from_exports

        for subsec in dict_subsector_to_energy_demand_proportions:
            vec = dict_subsector_to_energy_demand_proportions.get(subsec)
            vec *= vec_enfu_frac_from_others / (
                vec_enfu_total_demand + vec_enfu_imports
            )
        ...
        for modvar in dict_modvars_emission_to_allocate:
            ...
            for subsec in dict_entc_subsectors_to_emission_variables.keys():
                ...
                modvar_emission_cur_subsec = (
                    dict_entc_subsectors_to_emission_variables.get(subsec)
                )
                modvar_emission_cur_subsec = (
                    modvar_emission_cur_subsec.get(key_dict_emissions)
                    if (modvar_emission_cur_subsec is not None)
                    else None
                )
                vec_emissions = (
                    dict_subsector_to_energy_demand_proportions.get(subsec)
                    if (modvar_emission_cur_subsec is not None)
                    else None
                )
    ...
    def build_dummy_tech_cost(
        ...
        df_out.update({self.field_nemomod_mode: self.cat_enmo_gnrt}) if (
            cost_type == "variable"
        ) else None
        ...
        fields_for_multifield.append(self.field_nemomod_mode) if (
            cost_type == "variable"
        ) else None
    ...
    def conflict_resolution_func_vmmci(
        self,
        mm_tuple: tuple,
        approach: str = "swap",
        ...
        resolve_q = (
            (min_spec > max_spec)
            if (inequality_strength == "weak")
            else (min_spec >= max_spec)
        )
    ...
    def get_biogas_components(
        ...
        vec_enfu_minimum_fuel_frac_to_elec = (
            self.model_attributes.extract_model_variable(
                df_elec_trajectories,
                self.modvar_enfu_minimum_frac_fuel_used_for_electricity,
                expand_to_all_cats=True,
        ...
        vec_enfu_minimum_fuel_energy_to_electricity_biogas = (
            vec_enfu_total_energy_biogas * vec_enfu_minimum_fuel_frac_to_elec
        )
    ...
    def get_dict_tech_base_to_ccs(
        ...
        tab = attr_entc.table
        dict_out = tab[
            tab[field_ccs_tech].apply(clean_schema).isin(attr_entc.key_values)
        ]
    ...
    def get_dummy_fuel_description(
        self,
        ...
        """
        return_val = (
            "Dummy fuel used for accounting of production by dummy techs."
            if (return_type == "fuel")
    ...
    def get_dummy_fuel_tech_name(
        ...
        """
        return_val = (
            f"supply_{fuel}" if isinstance(fuel, str) else [f"supply_{x}" for x in fuel]
        )
        return return_val

    def get_dummy_fuel_techs(
        self,
        ...
        return_type = (
            return_type if (return_type in ["dict", "pd.DataFrame"]) else "dict"
        )

        fuels_from_dummy = (
            [] if drop_activity_ratio_fuels else attribute_fuel.key_values
        )
        ...
        if drop_activity_ratio_fuels:
            for fuel in attribute_fuel.key_values:
                ...
                use_dummy = (
                    (self.key_oar not in ar_vars.keys())
                    and (fuel != self.cat_enfu_elec)
        ...
        dict_fuels_dummy = dict(
            (x, self.get_dummy_fuel_tech_name(x)) for x in fuels_from_dummy
        )
        dict_fuels_dummy = (
            pd.DataFrame(
    ...
    def get_dummy_generation_fuel_techs(
        self,
        ...
        dict_return = dict(zip(fuels_keep, techs_dummy, strict=False))

        return dict_return

    def get_enfu_cats_with_high_dummy_tech_costs(
        self,
        ...
        return_type = (
            return_type if (return_type in ["fuels", "dummy_fuel_techs"]) else "fuels"
        )
        ...
        if not imports_only:
            ...
            cats_price_high += [
                x
                for x in dict_fuel_cats.keys()
                if dict_fuel_cats.get(x).get(self.key_oar) is not None
            ]
    ...
    def get_enfu_fuel_production_from_total_production(
        self,
        ...
        df_fuel_production = df_fuel_production[
    ...
    def get_enfu_non_entc_fuel_demands_from_annual_demand(
        self,
        ...
        modvar_enfu_demand = (
            ...
            if (modvar_enfu_demand is None)
            else modvar_enfu_demand
        )
        return_type = (
            "array_base"
            if (return_type not in ["array_base", "data_frame"])
            else return_type
        )
        ...
        arr_enfu_demand_non_entc = sf.vec_bounds(
            arr_enfu_full_demand - arr_enfu_exports,
            (0, np.inf),
        )
    ...
    def get_enfu_upstream_fuel_to_replace_downstream_fuel_consumption_map(
        self,
    ...
    def get_entc_cat_for_integration(
        self,
        cat_name: str,
    ) -> str:
        ...
        cat_name = cat_name if (cat_name in dict_cat_name_to_cat) else "waste"
    ...
    def get_entc_emissions_activity_ratio_comp_fp(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        list_entc_modvars_fp_ear = (
            ...
            if (list_entc_modvars_fp_ear is None)
            else list_entc_modvars_fp_ear
        )
    ...
    def get_entc_emissions_activity_ratio_comp_me(
        self,
        df_elec_trajectories: pd.DataFrame,
                ...
                for cat in cats_entc:
                    if cat in dict_tech_info.get("all_techs_me"):
                        (
                            ...
                            if (cat in dict_entc_me_tech_to_input_fuels)
                            else dict_entc_me_tech_to_input_fuels.update({cat: [fuel]})
                        )

                        # retrieve array
                        if cat not in dict_enfu_arrs_iar:
        ...
        for cat_entc in dict_entc_me_tech_to_input_fuels:
            cats_enfu_iar = dict_entc_me_tech_to_input_fuels.get(cat_entc)
            ind_entc = attribute_technology.get_key_value_index(cat_entc)

            dict_enfu_vec_fuel_total = dict(
                (dict_enfu_ef_to_gas.get(x), 0.0)
                for x in dict_enfu_arrs_efs_scaled_to_nemomod.keys()
            )
            ...
            for cat_enfu in cats_enfu_iar:
                ...
                upstream_q = (
                    cat_enfu
                    in self.get_enfu_upstream_fuel_to_replace_downstream_fuel_consumption_map(
                        attribute_fuel=attribute_fuel,
                ...
                vec_entc_iar = (
                    sf.vec_bounds(vec_entc_iar - 1, (0, np.inf))
                    if upstream_q
                    else vec_entc_iar
                )
                ...
                for modvar_ef in dict_enfu_arrs_efs_scaled_to_nemomod.keys():
                    ...
                    dict_enfu_vec_fuel_total[emission] += (
                        vec_enfu_fuel_ef * vec_entc_iar
                    )
    ...
    def get_entc_import_adjusted_msp(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        for fuel in dict_fuel_cats.keys():
            ...
            inds_entc = [
                attribute_technology.get_key_value_index(x) for x in cats_shared
            ]
    ...
    def get_entc_maxprod_increase_adjusted_msp(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        * Returns an np.ndarray wide by all ENTC categories and long by rows in
        ...
        for j in inds_modify:
            ...
            dict_entc_cat_to_position_base_prod_est.update({cat: i_last}) if (
                i_last >= 0
            ) else None
        ...
        fuels_adj = [
            k
            for k, v in self.dict_entc_fuel_categories_to_fuel_variables.items()
        ...
        for fuel in fuels_adj:
            ...
            cats_no_growth = [
                x for x in cats_modvar if x in dict_entc_cat_to_position_base_prod_est
            ]
            ...
            inds_no_growth = [
                attribute_technology.get_key_value_index(x) for x in cats_no_growth
            ]
            inds_response = [
                attribute_technology.get_key_value_index(x) for x in cats_response
            ]
            ...
            row_inds_no_growth = [
                dict_entc_cat_to_position_base_prod_est.get(x) for x in cats_no_growth
            ]
            ...
            for i, j in enumerate(inds_no_growth):
                ...
                fracs_new *= 1 + sf.vec_bounds(
                    arr_entc_maxprod_msp_increase[(row + 1) :, j],
                    (0, np.inf),
                )
                ...
                arr_entc_activity_limits[(row + 1) :, j] = (
                    fracs_new * vec_prod_est_cur_fuel[(row + 1) :]
                )
    ...
    def get_gnrl_ccf_hydropower_factor_df(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        field_gnrl_ccf_hydropower = [
            x
            for x in df_gnrl_ccf_hydropower.columns
            if x
            not in [
                self.model_attributes.dim_time_period,
                self.field_nemomod_technology,
    ...
    def get_integrated_waste_emissions_activity_ratio(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_technology: Union[AttributeTable, None] = None,
        attribute_time_period: Union[AttributeTable, None] = None,
        ...
        vec_enfu_total_energy_waste, vec_enfu_min_energy_to_elec_waste, dict_efs = (
    ...
    def get_nemomod_optimizer(
        ...
        optimizer = self.julia_main.JuMP.Model(self.solver_module.Optimizer)

        # set some generic properties
        self.julia_main.JuMP.set_time_limit_sec(optimizer, self.solver_time_limit)
        self.julia_main.JuMP.set_silent(optimizer)

        ##  SOLVER SPECIFIC PROPERTIES (setup from config for later)

        # gams/cplex parameters
        if solver == "gams_cplex":
            self.julia_main.JuMP.set_optimizer_attribute(optimizer, "Solver", "cplex")

        # gurobi parameters
        if solver == "gurobi":
            # see https://www.gurobi.com/documentation/9.5/refman/numericfocus.html#parameter:NumericFocus
            self.julia_main.JuMP.set_optimizer_attribute(optimizer, "NumericFocus", 2)
    ...
    def get_variable_cost_fuels_gravimetric_density(
        self,
        df_elec_trajectories: pd.DataFrame,
        override_time_period_transformation: bool = False,
        regions: Union[List[str], None] = None,
        ...
        df_out[self.field_nemomod_technology] = df_out[
            self.field_nemomod_technology
        ].apply(self.get_dummy_fuel_tech_name)
        df_out.drop(["price", "density"], axis=1, inplace=True)

        return df_out

    def get_variable_cost_fuels_volumetric_density(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ):
    ...
    def get_waste_energy_components(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_technology: AttributeTable = None,
        ...
        if tuple_waso_incineration is not None:
            # get mass of waste incinerated,
            modvar_waso_mass_incinerated, array_waso_mass_incinerated = (
                tuple_waso_incineration
            )
            ...
            vec_enfu_minimum_fuel_energy_to_electricity_waste = (
                vec_enfu_total_energy_waste * vec_enfu_minimum_fuel_frac_to_elec
            )
    ...
    def format_model_variable_as_nemomod_table(
        self,
        df_elec_trajectories: pd.DataFrame,
        modvar: str,
        table_nemomod: str,
        ...
        **kwargs: passed to ModelAttributes.extract_model_variable(#)
        ...
        scalar_to_nemomod_units = (
            1.0 if not sf.isnumber(scalar_to_nemomod_units) else scalar_to_nemomod_units
        )
        ...
        exchange_year_tp &= (
            self.model_attributes.dim_time_period in df_elec_trajectories.columns
        )
        if exchange_year_tp:
    ...
    def format_dummy_tech_description_from_fuel(
        self,
        fuel: str,
    ) -> str:
    ...
    def verify_min_max_constraint_inputs(
        self,
        df_max: pd.DataFrame,
        df_min: pd.DataFrame,
        field_max: str,
        ...
        fields_shared = [
            x for x in fields_shared if x not in [field_min, field_max, field_id]
        ]
        ...
        field_maxm = (
            f"{field_max}_{suffix_max}" if (field_max == field_min) else field_max
        )
        field_minm = (
            f"{field_min}_{suffix_min}" if (field_max == field_min) else field_min
        )
    ...
    def format_nemomod_attribute_table_emission(
        self,
        ...
        fields_ord = [
            x for x in self.fields_nemomod_sort_hierarchy if (x in df_out.columns)
        ]
    ...
    def format_nemomod_attribute_table_fuel(
        self,
        ...
        # set some defaults
        attribute_fuel = (
            self.get_attribute_enfu() if (attribute_fuel is None) else attribute_fuel
        )
        ...
        df_out = pd.concat(
            [df_out[fields_ext], df_append[fields_ext]],
            axis=0,
        ).reset_index(drop=True)
    ...
    def format_nemomod_attribute_table_mode_of_operation(
        self,
    ...
    def format_nemomod_attribute_table_node(
        self,
    ...
    def format_nemomod_attribute_table_region(
        self,
    ...
    def format_nemomod_attribute_table_storage(
        self,
    ...
    def format_nemomod_attribute_table_technology(
        self,
        attribute_fuel: Union[AttributeTable, None] = None,
        attribute_technology: Union[AttributeTable, None] = None,
        dict_rename: dict = None,
        ...
        df_out_dummies[self.field_nemomod_description] = df_out_dummies[
            self.field_nemomod_fuel
    ...
    def format_nemomod_attribute_table_year(
        self,
        time_period_as_year: bool = None,
    ) -> Dict[str, pd.DataFrame]:
    ...
    def format_nemomod_table_annual_emission_limit(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_emission: AttributeTable = None,
        ...
        for modvar in enumerate(modvars_limit):
            ...
            vec_exogenous_emissions = np.sum(
                np.array(df_elec_trajectories[fields]),
                axis=1,
            )
            ...
            vec_emission_limit_out = sf.vec_bounds(
                vec_emission_limit - vec_exogenous_emissions,
                (0, np.inf),
            )
    ...
    def format_nemomod_table_availability_factor(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        regions_keep = (
            (
                regions_keep
                & set(df_reference_availability_factor[self.field_nemomod_region])
            )
        ...
        fields_melt = [
            x
            for x in df_reference_availability_factor.columns
            if (x in attribute_technology.key_values)
        ]
        df_out = pd.melt(
    ...
    def format_nemomod_table_capacity_to_activity_unit(
        self,
        regions: Union[List[str], None] = None,
    ...
    def format_nemomod_table_costs_technology(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
        ...
        df_entc_variable_costs[self.model_attributes.dim_time_period] = (
            df_elec_trajectories[self.model_attributes.dim_time_period]
        )
        ...
        for cat_entc in enumerate(cats_df_variable_costs):
            ...
            df_entc_variable_costs[field_varcost] = (
                np.array(df_entc_variable_costs[field_varcost])
        ...
        for table_name in list(dict_return.keys()):
            ...
            price_high = max(
                np.round(max(df_tmp[self.field_nemomod_value]) * 2) * 1000 + 10,
                minimum_dummy_price,
            )
            vec_high_cost_bool = np.array(
                [
    ...
    def format_nemomod_table_costs_storage(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_default_parameters(
        self,
        ...
        dict_repl = attribute_nemomod_table.field_maps[
            f"{attribute_nemomod_table.key}_to_{field_default_values}"
        ].copy()
        dict_repl.update(
    ...
    def format_nemomod_table_discount_rate(
        self,
        regions: Union[List[str], None] = None,
        return_type: str = "table",
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_emissions_activity_ratio(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
        attribute_technology: Union[AttributeTable, None] = None,
        ...
        cat_entc_pp_waste = self.get_entc_cat_for_integration("wste")
        cats_entc_ordered = [
            x for x in attr_entc.key_values if x in dict_pp_tech_to_fuel.keys()
        ]
        inds_enfu_extract = [
            attr_enfu.get_key_value_index(dict_pp_tech_to_fuel.get(x))
            for x in cats_entc_ordered
        ]
        ...
        for modvars in enumerate(list_modvars_enfu_to_tech):
            ...
            # expand to tech
            arr_entc_tmp = (
                self.model_attributes.merge_array_var_partial_cat_to_array_all_cats(
                    arr_enfu_tmp,
                    None,
                    missing_vals=0.0,
    ...
    def format_nemomod_table_fixed_cost(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_interest_rate_storage(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_interest_rate_technology(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_input_activity_ratio(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
        attribute_technology: Union[AttributeTable, None] = None,
        ...
        all_techs_generate = [
            x
            for x in attribute_technology.key_values
            if x not in dict_tech_to_storage.keys()
        ]
        ...
        for fuel in dict_fuel_cats.keys():
            ...
            if modvar_iar is not None:
                ...
                for i in df_tmp.groupby(self.field_nemomod_technology):
                    ...
                    ids_filt += (
                        list(df[self.field_nemomod_id])
                        if not ((len(unique_vals) == 1) and (0.0 in unique_vals))
                        else []
                    )
                ...
                df_tmp[self.field_nemomod_mode] = df_tmp[
                    self.field_nemomod_technology
                ].replace(dict_tech_to_mode)
        ...
        df_transmission_loss.rename(
            columns={self.field_nemomod_value: field_transmission_merge},
            inplace=True,
        )
    ...
    def format_nemomod_table_min_share_production(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
        attribute_technology: Union[AttributeTable, None] = None,
        ...
        if (tuple_enfu_production_and_demands is None) and (
        ...
        arr_enfu_import_fractions_adj = np.nan_to_num(
            tuple_enfu_production_and_demands[0]
            / (
                tuple_enfu_production_and_demands[0]
                + tuple_enfu_production_and_demands[2]
            ),
            nan=1.0,
            posinf=1.0,
        )
        ...
        df_fracs_adj = [
            df_elec_trajectories[
                [
                    x
                    for x in df_elec_trajectories.columns
                    if x in self.model_attributes.sort_ordered_dimensions_of_analysis
                ]
            ].copy(),
        ]
        df_fracs_adj += [
        ...
        vec_entc_elec_demand_frac_from_tech_lower_limit = (
            self.estimate_production_share_from_activity_limits(
                df_elec_trajectories,
                tuple_enfu_production_and_demands=tuple_enfu_production_and_demands,
            )
        )
        ...
        arr_enfu_import_fractions_adj_for_msp_adj[:, self.ind_enfu_elec] += (
            vec_entc_elec_demand_frac_from_tech_lower_limit
        )
    ...
    def format_nemomod_table_min_storage_charge(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_storage: AttributeTable = None,
    ...
    def format_nemomod_table_minimum_utilization(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_model_period_emission_limit(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_model_period_exogenous_emission(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_operational_life(
        self,
        attribute_fuel: AttributeTable = None,
        attribute_storage: AttributeTable = None,
        attribute_technology: AttributeTable = None,
        ...
        # get the life time
        dict_techs_to_operational_life = (
            self.model_attributes.get_ordered_category_attribute(
                self.model_attributes.subsec_name_entc,
                "operational_life",
                return_type=dict,
    ...
    def format_nemomod_table_output_activity_ratio(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
        attribute_technology: Union[AttributeTable, None] = None,
    ...
    def format_nemomod_table_re_min_production_target(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
    ...
    def format_nemomod_table_re_tag_technology(
        self,
    ...
    def format_nemomod_table_reserve_margin(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_reserve_margin_tag_fuel(
        self,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
        """Format the ReserveMargin input table for NemoMod based on SISEPUEDE
    ...
    def format_nemomod_table_reserve_margin_tag_technology(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_residual_capacity(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_residual_storage_capacity(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
    ...
    def estimate_production_share_from_activity_limits(
        self,
        df_elec_trajectories: pd.DataFrame,
        tuple_enfu_production_and_demands: Union[Tuple[pd.DataFrame], None] = None,
    ) -> np.ndarray:
        ...
        arr_enfu_production[:, self.ind_enfu_elec] = np.nan_to_num(
            arr_enfu_production[:, self.ind_enfu_elec]
            / (1 - arr_transmission_loss[:, self.ind_enfu_elec]),
            nan=0.0,
            posinf=0.0,
        )
    ...
    def format_nemomod_table_specified_annual_demand(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: Union[AttributeTable, None] = None,
        attribute_time_period: Union[AttributeTable, None] = None,
        ...
        df_enfu_production = pd.DataFrame(
            arr_enfu_production,
            columns=attribute_fuel.key_values,
        )
        fields_all_zero = [
            x
            for x in df_enfu_production.columns
            if (len(set(df_enfu_production[x])) == 1)
            and max(list(df_enfu_production[x])) == 0
        ]
    ...
    def format_nemomod_table_specified_demand_profile(
        ...
        df_out = (
            df_out[df_out[self.field_nemomod_region].isin(regions_keep)]
            if (self.field_nemomod_region in df_out.columns)
            else df_out
        )
        ...
        sf.check_fields(
            df_out,
            fields_required,
            msg_prepend="Error in format_nemomod_table_specified_demand_profile: required fields ",
        )
        ...
        df_time_slices = attribute_time_slice.table[["time_slice", "weight"]].rename(
            columns={
    ...
    def format_nemomod_table_storage_attributes(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_storage: AttributeTable = None,
        field_attribute_min_charge: str = "minimum_charge_fraction",
    ...
    def format_nemomod_table_storage_start_level(
        self,
        df_elec_trajectories: pd.DataFrame,
    ) -> pd.DataFrame:
    ...
    def format_nemomod_table_technology_from_and_to_storage(
        self,
        attribute_storage: AttributeTable = None,
        attribute_technology: AttributeTable = None,
        regions: Union[List[str], None] = None,
    ...
    def format_nemomod_table_tsgroup_tables(
        self,
        ...
        df_time_slice = (
            attribute_time_slice.table.copy().drop_duplicates().reset_index(drop=True)
        )
        ...
        df_ltsgroup.rename(
            columns={"time_slice": self.field_nemomod_time_slice},
            inplace=True,
        )
        fields_ext = [
            self.field_nemomod_id,
        ...
        df_year_split = pd.merge(
            df_year_split,
            df_ltsgroup[[self.field_nemomod_time_slice, self.field_nemomod_id]],
            how="left",
        )
        ...
        df_tgs = pd.merge(
            ...
            df_year_split[
                df_year_split[self.field_nemomod_year]
                == min(df_year_split[self.field_nemomod_year])
            ][[self.field_nemomod_time_slice, self.field_nemomod_value]],
            how="left",
        ).sort_values(by=[self.field_nemomod_id])
        # some dictionaries
        dict_field_to_attribute = {
            self.field_nemomod_tg1: "ts_group_1",
            self.field_nemomod_tg2: "ts_group_2",
        }
        dict_tg = {}
        ...
        for fld in [self.field_nemomod_tg1, self.field_nemomod_tg2]:
            ...
            df_tgs_out = (
                pd.merge(df_tgs_out, attr_cur)
                .sort_values(by=[self.field_nemomod_order])
                .reset_index(drop=True)
            )
        ...
        dict_return = {
            ...
            self.model_attributes.table_nemomod_ts_group_1: dict_tg[
                self.field_nemomod_tg1
            ],
            self.model_attributes.table_nemomod_ts_group_2: dict_tg[
                self.field_nemomod_tg2
            ],
    ...
    def format_nemomod_table_total_capacity_tables(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
        ...
        if dfs_verify is not None:
            dict_return.update(
                {
                    self.model_attributes.table_nemomod_total_annual_max_capacity: dfs_verify[
                        0
                    ],
                    self.model_attributes.table_nemomod_total_annual_min_capacity: dfs_verify[
                        1
                    ],
                },
            )
        ...
        if dfs_verify is not None:
            dict_return.update(
                {
                    self.model_attributes.table_nemomod_total_annual_max_capacity_investment: dfs_verify[
                        0
                    ],
                    self.model_attributes.table_nemomod_total_annual_min_capacity_investment: dfs_verify[
                        1
                    ],
                },
            )
    ...
    def format_nemomod_table_total_capacity_storage_tables(
        self,
        df_elec_trajectories: pd.DataFrame,
        regions: Union[List[str], None] = None,
    ) -> pd.DataFrame:
        ...
        if dfs_verify is not None:
            dict_return.update(
                {
                    self.model_attributes.table_nemomod_total_annual_max_capacity_storage: dfs_verify[
                        0
                    ],
                    self.model_attributes.table_nemomod_total_annual_min_capacity_storage: dfs_verify[
                        1
                    ],
                },
            )
        ...
        if dfs_verify is not None:
            dict_return.update(
                {
                    self.model_attributes.table_nemomod_total_annual_max_capacity_investment_storage: dfs_verify[
                        0
                    ],
                    self.model_attributes.table_nemomod_total_annual_min_capacity_investment_storage: dfs_verify[
                        1
                    ],
                },
            )
    ...
    def format_nemomod_table_total_technology_activity_lower_limit(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: AttributeTable = None,
        attribute_technology: AttributeTable = None,
    ...
    def format_nemomod_table_total_technology_activity_upper_limit(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_fuel: AttributeTable = None,
        attribute_technology: AttributeTable = None,
    ...
    def get_total_technology_activity_lower_limit_no_msp_adjustment(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_technology: AttributeTable = None,
        regions: Union[List[str], None] = None,
        ...
        vec_enfu_total_energy_supply_biogas, vec_enfu_min_energy_to_elec_biogas = (
        ...
        if return_type == "CapacityCheck":
            df_out[self.field_nemomod_value] = (
                np.array(df_out[self.field_nemomod_value]) * scalar_energy_to_power_cur
            )
    ...
    def get_total_technology_activity_upper_limit_no_msp_adjustment(
        self,
        df_elec_trajectories: pd.DataFrame,
        attribute_technology: AttributeTable = None,
        regions: Union[List[str], None] = None,
    ...
    def overwrite_ccs_variables(
        ...
        modvar_mix = (
            self.modvar_ccs_achievement_frac if modvar_mix is None else modvar_mix
        )
    ...
    def update_ttal_dictionary_with_limit_from_msp(
        self,
        df_elec_trajectories: pd.DataFrame,
        ...
        **kwargs,
        ...
        if arr_entc_activity_limits is not None:
            ...
            df_entc_activity_limits_append[self.model_attributes.dim_time_period] = (
                list(df_elec_trajectories[self.model_attributes.dim_time_period])
            )
    ...
    def format_dataframe_from_retrieval(
        self,
        df_in: pd.DataFrame,
        field_pivot: str,
        ...
        field_values = (
            self.field_nemomod_value if (field_values is None) else field_values
        )
        time_period_as_year = (
            self.nemomod_time_period_as_year
            if (time_period_as_year is None)
    ...
    def retrieve_nemomod_table_discounted_capital_invesment(
        self,
    ...
    def retrieve_nemomod_table_discounted_capital_invesment_storage(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        table_name: str = None,
    ...
    def retrieve_nemomod_table_discounted_operating_cost(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        table_name: str = None,
    ...
    def retrieve_nemomod_table_discounted_operating_cost_storage(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        table_name: str = None,
    ...
    def retrieve_nemomod_table_emissions_by_technology(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        table_name: str = None,
    ...
    def retrieve_nemomod_fuel_sectoral_demands_and_imports(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        ...
        df_use = (
            df_use[~df_use[self.field_nemomod_technology].isin(all_techs_st)]
            if (all_techs_st is not None)
            else df_use
        )
        ...
        df_demands = sqlutil.sql_table_to_df(engine, table_name_demands).drop(
            self.field_nemomod_solvedtm,
            axis=1,
        )
        field_val_demands = f"{self.field_nemomod_value}_DEMANDS"
        df_demands.rename(
            columns={self.field_nemomod_value: field_val_demands},
            inplace=True,
        )
        ...
        dict_cats_entc_import = dict(
            ...
            for x in cats_enfu_import
        )
        inds = df_use[self.field_nemomod_technology].isin(
            [
        ...
        df_imports = (
            df_use[
                df_use[self.field_nemomod_technology].isin(dict_cats_entc_import.keys())
            ]
            .copy()
            .reset_index(drop=True)
        )
        df_imports[self.field_nemomod_fuel] = df_imports[
            self.field_nemomod_technology
        ...
        df_use[self.field_nemomod_value] = sf.vec_bounds(
            np.array(df_use[self.field_nemomod_value])
            - np.array(df_use[field_val_demands]),
            (0, np.inf),
        )
        ...
        arr_transmission_loss_frac = (
            None
            if arr_transmission_loss_frac.shape
            != (len(df_out_enfu_demand_entc), attribute_fuel.n_key_values)
    ...
    def retrieve_nemomod_table_total_capacity(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        table_name: str = None,
    ...
    def retrieve_nemomod_tables_fuel_production_demand_and_trade(
        self,
        engine: sqlalchemy.engine.Engine,
        vector_reference_time_period: Union[list, np.ndarray],
        df_elec_trajectories: Union[pd.DataFrame, None],
        ...
        * The mass balance equations are as follows:
        ...
        table_name_demand_annual = (
            ...
            else table_name_demand_annual
        )
        table_name_production_by_technology = (
            self.model_attributes.table_nemomod_production_by_technology
            if (table_name_production_by_technology is None)
            else table_name_production_by_technology
        )
        dict_tech_info = self.get_tech_info_dict(
            attribute_fuel=attribute_fuel,
            attribute_technology=attribute_technology,
        ...
        df_enfu_demands_entc, df_enfu_imports = (
            self.retrieve_nemomod_fuel_sectoral_demands_and_imports(
                engine,
                vector_reference_time_period,
        ...
        arr_enfu_demands = (
            arr_enfu_demands_no_entc
            + arr_enfu_demand_entc * scalar_enfu_energy_demand_entc_to_nemo_units
        )
        ...
        df_enfu_costs = self.model_attributes.array_to_df(
            arr_entc_total_fuel_value
            * arr_enfu_demand_entc
            * scalar_enfu_demand_entc_to_config_energy,
            self.modvar_enfu_value_of_fuel_entc,
            reduce_from_all_cats_to_specified_cats=True,
        )
        ...
        scalar_arg = (
            ...
            else self.modvar_entc_nemomod_production_by_technology
        )
    ...
    def retrieve_and_pivot_nemomod_table(
        self,
        ...
        field_pivot = (
            self.field_nemomod_technology if (field_pivot is None) else field_pivot
        )
        ...
        dict_repl = {}
        if (subsec == self.subsec_name_enfu) and (
            field_pivot != self.field_nemomod_fuel
        ):
            dict_repl = dict_tech_info.get("dict_dummy_techs_to_fuels")
        elif (subsec == self.subsec_name_enst) and (
            field_pivot != self.field_nemomod_storage
        ):
        ...
        dict_cats_to_varname = dict(zip(dict_cats_to_varname, varnames, strict=False))
    ...
    def generate_input_tables_for_sql(
        self,
        df_elec_trajectories: pd.DataFrame,
        df_reference_availability_factor: pd.DataFrame,
        ...
        for table in dict_out:
            dict_dtype = {}
            for k in self.dict_fields_nemomod_to_type.keys():
                dict_dtype.update({k: self.dict_fields_nemomod_to_type[k]}) if (
                    k in dict_out[table].columns
                ) else None
    ...
    def retrieve_output_tables_from_sql(
        self,
        engine: sqlalchemy.engine.Engine,
        df_elec_trajectories: pd.DataFrame,
        tuple_enfu_production_and_demands: Union[Tuple[pd.DataFrame], None] = None,
    ...
    def project(
        ...
        * Either engine or fp_database must be specified to run project. If both
        ...
        df_elec_trajectories, df_se_internal_shared_variables = (
            self.model_socioeconomic.project(df_elec_trajectories)
        )
        ...
        dict_ref_tables = (
            self.dict_nemomod_reference_tables
            if (dict_ref_tables is None)
            else dict_ref_tables
        )
        sf.check_keys(
            dict_ref_tables,
            [
        ...
        if not os.path.exists(fp_database):
            self._log(
                f"\tPath to temporary NemoMod database '{fp_database}' not found. Creating...",
                type_log="info",
            )
            self.julia_main.NemoMod.createnemodb(fp_database)
        ...
        vector_calc_time_periods = (
            ...
            if (vector_calc_time_periods is None)
                ...
                x for x in attr_time_period.key_values if x in vector_calc_time_periods
            ]
        )
        ...
        msg = (
            ...
            if successful_run_q
            else f"NemoMod run failed with result {result}. Populating missing data with value {missing_vals_on_error}."
        )
        ...
        arr_enfu_total_unused_fuel_exported = np.zeros(
            (
                len(df_elec_trajectories),
                self.model_attributes.get_attribute_table(
                    self.subsec_name_enfu,
        ...
        arr_enfu_total_unused_fuel_exported[:, self.ind_enfu_bgas] = sf.vec_bounds(
            vec_enfu_total_energy_supply_biogas - vec_used_bgas,
            (0, np.inf),
        )
        arr_enfu_total_unused_fuel_exported[:, self.ind_enfu_wste] = sf.vec_bounds(
            vec_enfu_total_energy_supply_waste - vec_used_wste,
            (0, np.inf),
        )
