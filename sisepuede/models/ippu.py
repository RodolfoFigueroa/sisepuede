...
class IPPU:
    ...
    def _initialize_subsector_vars_ippu(
        self,
    ) -> None:
        ...
        * self.modvar_ippu_****
        ...
        self.modvar_ippu_capture_efficacy_co2 = (
            "Industrial :math:\\text{CO}_2 Capture Efficacy"
        )
        self.modvar_ippu_capture_prevalence_co2 = (
            "Industrial :math:\\text{CO}_2 Capture Prevalence"
        )
        ...
        self.modvar_ippu_gas_captured_co2 = (
            ":math:\\text{CO}_2 Captured in Industrial Processes and Product Use"
        )
        ...
        self.modvar_ippu_ef_ch4_per_prod_process = (
            ":math:\\text{CH}_4 Production Process Emission Factor"
        )
        ...
        self.modvar_ippu_ef_co2_per_prod_produse = (
            ":math:\\text{CO}_2 Product Use Emission Factor"
        )
        self.modvar_ippu_ef_co2_per_prod_process_clinker = (
            ":math:\\text{CO}_2 Clinker Production Process Emission Factor"
        )
        ...
        self.modvar_ippu_ef_n2o_per_gdp_process = (
            ":math:\\text{N}_2\\text{O} GDP Production Process Emission Factor"
        )
        self.modvar_ippu_ef_n2o_per_prod_process = (
            ":math:\\text{N}_2\\text{O} Production Process Emission Factor"
        )
        self.modvar_ippu_ef_nf3_per_prod_process = (
            ":math:\\text{NF}_3 Production Process Emission Factor"
        )
        ...
        self.modvar_ippu_ef_sf6_per_gdp_process = (
            ":math:\\text{SF}_6 GDP Production Process Emission Factor"
        )
        self.modvar_ippu_ef_sf6_per_prod_process = (
            ":math:\\text{SF}_6 Production Process Emission Factor"
        )
        # HFCs
        ...
        self.modvar_ippu_emissions_other_nonenergy_co2 = (
            "Initial Other Non-Energy :math:\\text{CO}_2 Emissions"
        )
        self.modvar_ippu_emissions_process_ch4 = (
            ":math:\\text{CH}_4 Emissions from Industrial Production Processes"
        )
        self.modvar_ippu_emissions_process_co2 = (
            ":math:\\text{CO}_2 Emissions from Industrial Production Processes"
        )
        self.modvar_ippu_emissions_produse_co2 = (
            ":math:\\text{CO}_2 Emissions from Industrial Product Use"
        )
        self.modvar_ippu_emissions_process_dodecafluoropentane = (
            "Dodecafluoropentane Emissions from Industrial Production Processes"
        )
        self.modvar_ippu_emissions_produse_hcfc141b = (
            "HCFC-141b Emissions from Industrial Product Use"
        )
        self.modvar_ippu_emissions_produse_hcfc142b = (
        ...
        self.modvar_ippu_emissions_process_n2o = (
            ":math:\\text{N}_2\\text{O} Emissions from Industrial Production Processes"
        )
        self.modvar_ippu_emissions_process_nf3 = (
            ":math:\\text{NF}_3 Emissions from Industrial Production Processes"
        )
    ...
    def calculate_emissions_by_gdp_and_production(
        self,
        ...
        for modvar in dict_simple_efs:
            ...
            if isinstance(array_production, np.ndarray):
                for modvar_ef_prod in all_modvar_ef_prod:
                    ...
                    array_emission += (
                        array_emission_cur * array_production / scalar_ippu_mass
                    )
            ...
            if include_carbon_capture & (gas == "co2"):
                ...
                array_emission_captured_cur = (
                    array_emission * array_emission_frac_captured
                )
    ...
    def project_hh_construction(
        ...
        if len(vec_average_lifetime_hh) != len(vec_hh):
            self._log(
                "Warning in `IPPU.project_hh_construction()`: average lifetime of housholds and number of households should have the same length vectors. Setting lifetime to repeat of final value.",
                type_log="warning",
            )
            vec_average_lifetime_hh = np.conactenate(
                [
                    vec_average_lifetime_hh,
                    np.array(
                        [
        ...
        for i in range(n_projection_time_periods):
            ...
            if ind_lifetime_old_stock >= 0:
                old_stock = (
                    vec_old_housing_stock_rev[ind_lifetime_old_stock]
                    if (ind_lifetime_old_stock < len(vec_old_housing_stock_rev))
                    else 0
                )
            ...
            vec_est_new_builds[i] = (
                old_stock_refreshed + vec_new_housing_stock_changes[i]
            )

        return vec_est_new_builds

    def project_industrial_production(
        self,
        df_ippu_trajectories: pd.DataFrame,
        ...
        arr_ippu_materials_required_baseline = np.outer(
            vec_ippu_housing_construction,
            arr_ippu_materials_required[0],
        )
        arr_ippu_materials_required = (
            arr_ippu_materials_required.transpose() * vec_ippu_housing_construction
        ).transpose()
        arr_ippu_materials_required_change = (
            arr_ippu_materials_required - arr_ippu_materials_required_baseline
        )

        # adjust production and net imports
        array_ippu_ind_balance = (
            array_ippu_ind_prod + arr_ippu_materials_required_change
        )
        array_ippu_ind_prod = sf.vec_bounds(array_ippu_ind_balance, (0, np.inf))
        array_ippu_change_to_net_imports_cur = (
            array_ippu_ind_balance - array_ippu_ind_prod
        )

        return array_ippu_ind_prod, array_ippu_change_to_net_imports_cur

    def get_production_with_recycling_adjustment(
        self,
        df_ippu_trajectories: pd.DataFrame,
        vec_rates_gdp: np.ndarray,
        dict_dims: dict = None,
        ...
        array_ippu_production, array_ippu_change_net_imports = (
            self.project_industrial_production(
                df_ippu_trajectories,
                vec_rates_gdp,
                dict_dims,
                n_projection_time_periods,
        ...
        if array_ippu_recycled is not None:
            ...
            factor_ippu_waso_recycle_to_ippu_recycle = (
                self.model_attributes.get_variable_unit_conversion_factor(
                    modvar_waste_total_recycled,
                    modvar_prod_qty_init,
                    "mass",
            ...
            cats_ippu_to_recycle_ordered = (
                self.model_attributes.get_ordered_category_attribute(
                    self.subsec_name_ippu,
                    "target_cat_industry_to_adjust_with_recycling",
                )
            )
            vec_ippu_cats_to_adjust_from_recycling = [
                clean_schema(x) for x in cats_ippu_to_recycle_ordered
            ]

            # get indexes of of valid categories specified for recycling adjustments
            w = [
                i
                for i in range(len(vec_ippu_cats_to_adjust_from_recycling))
                if (
                    (vec_ippu_cats_to_adjust_from_recycling[i] != "none")
                    and (
                        vec_ippu_cats_to_adjust_from_recycling[i]
                        in attr_ippu.key_values
                    )
                )
            ]
            ...
            if len(w) > 0:
                ...
                array_ippu_maxiumum_recycling_ratio = (
                    self.model_attributes.extract_model_variable(
                        df_ippu_trajectories,
                        self.modvar_ippu_max_recycled_material_ratio,
                ...
                array_ippu_production_base = array_ippu_production * (
                    1 - array_ippu_maxiumum_recycling_ratio
                )
                array_ippu_production = (
                    array_ippu_production * array_ippu_maxiumum_recycling_ratio
                    - array_ippu_recycled_waste_adj
                )
                ...
                array_ippu_production = (
                    sf.vec_bounds(array_ippu_production, (0, np.inf))
        ...
        arr_ippu_ratio_of_production_to_wood_harvesting = (
            self.model_attributes.extract_model_variable(
                df_ippu_trajectories,
                modvar_ratio_of_production_to_harvested_wood,
                expand_to_all_cats=True,
    ...
    def project(
        ...
        df_ippu_trajectories, df_se_internal_shared_variables = (
            self.model_socioeconomic.project(df_ippu_trajectories)
        )
        ...
        array_ippu_net_imports_clinker *= (
            self.model_attributes.get_variable_unit_conversion_factor(
                self.modvar_ippu_net_imports_clinker,
                self.modvar_ippu_qty_total_production,
                "mass",
        ...
        array_ippu_production_clinker = sf.vec_bounds(
            array_ippu_production_clinker * array_ippu_production
            - array_ippu_net_imports_clinker,
            (0, np.inf),
        )
        ...
        array_ippu_useinit_nonenergy_fuel = (
            self.model_attributes.extract_model_variable(
                df_ippu_trajectories,
        ...
        array_ippu_gdp_scalar_produse = (
            array_ippu_gdp_scalar_produse.transpose()
            * np.concatenate([np.ones(1), np.cumprod(1 + vec_rates_gdp)])
        ).transpose()
        ...
        array_ippu_emissions_other_nonenergy_co2 = (
            self.model_attributes.extract_model_variable(
                df_ippu_trajectories,
                self.modvar_ippu_emissions_other_nonenergy_co2,
                expand_to_all_cats=True,
        ...
        array_ippu_emissions_other_nonenergy_co2 = (
            array_ippu_emissions_other_nonenergy_co2[0]
