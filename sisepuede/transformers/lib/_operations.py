...
def build_default_general_config_dict(
    ...
    # clean tuples
    dict_vir = dict(
        (k, list(v) if isinstance(v, tuple) else v) for k, v in dict_vir.items()
    )
    ...
    (dict_out.get(trs._DICT_KEYS.get("general")).update(dict_vir))
    ...
    (dict_out.get(trs._DICT_KEYS.get("baseline")).update(dict_update))

    (dict_out.get(trs._DICT_KEYS.get("baseline")).update(dict_vir2))
...
def build_default_strategies(
    ...
    **kwargs,
    ...
    attr_sector = (
        transformations.transformers.model_attributes.get_sector_attribute_table()
    )
    ...
    dict_transformation_codes_by_transformer_code = (
        transformations.get_transformation_codes_by_transformer_code()
    )
    ...
    dict_transformer_codes_by_sector = (
        transformations.transformers.get_transformer_codes_by_sector()
    )
    ...
    keys_sector = [
        attr_sector.get_attribute(k, "sector") for k in attr_sector.key_values
    ]
    ...
    # strat_id = sid_factor*factor_sectoral_multiplier + baseline_strategy_id
    description = "All actions (unique by transformer)"
...
def _cn_default_strategy_sector_composite(
    ...
    attr_sector = (
        (transformations.transformer.model_attributes.get_sector_attribute_table())
...
def _cn_default_strategy_singleton(
    ...
    # modify code/name
    code = transformation.code.replace(
...
def code_to_file_name(
    ...
    out = ma.clean_schema(
        code.replace(trs._MODULE_CODE_SIGNATURE, "").strip(":").replace(":", "_"),
    )
...
def spawn_args_dict(
    ...
    dict_out = dict(zip(args[-n:], dflts, strict=False))
    ...
    if sf.islistlike(args_ignore):
        for k in args_ignore:
            dict_out.pop(k) if (k in dict_out) else None
