...
def check_implementation_ramp(
    ...
    out = vec_implementation_ramp if not return_obj else obj.vec_implementation_ramp
...
def get_time_period(
    ...
    return_val = (
        ...
        if (return_type == "min")
        else max(attr_time_period.key_values)
    )
...
def prepare_demand_scalars(
    ...
    modvars = (
        ...
        else (list(modvars) if sf.islistlike(modvars) else None)
    )
    ...
    # group by region
    key_region = model_attributes.dim_region if key_region is None else key_region
    df_out = (
    ...
    for i, df in enumerate(df_out):
        row = df[df[time_periods.field_time_period].isin([tp_min])]
...
def transformation_general(
    ...
    for modvar in modvars:
        ...
        bounds = (
            None if not (isinstance(bounds, tuple) and len(bounds) == 2) else bounds
        )
        verified_modvar = (
        ...
        categories_source = (
            None if not isinstance(categories_source, list) else categories_source
        )
        ...
        verified_modvar = (
            ...
            if (
                magnitude_type
                in ["transfer_value", "transfer_value_to_acheieve_magnitude"]
            )
            else verified_modvar
        )
        verified_modvar = (
        ...
        tp_baseline = (
            max(attr_time_period.key_values)
            if (tp_baseline not in attr_time_period.key_values)
            else tp_baseline
        )
        ...
        if verified_modvar:
            ...
            if verify_categories:
                ...
                if categories_source is not None:
                    categories_source = [
                        x for x in categories_source if x in attr.key_values
                    ]
                    categories_source = (
                        None if (len(categories_source) == 0) else categories_source
                    )

                if categories_target is not None:
                    categories_target = dict(
                        (k, v)
                        for k, v in categories_target.items()
                        if k in attr.key_values
                    )
                    categories_target = (
                        None
                        if (sum(list(categories_target.values())) != 1.0)
                        else categories_target
                    )
        ...
        if region in regions_apply:
            for modvar in modvars:
                ...
                if magnitude_type in [
                    ...
                    total_target_initial = (
                        sum(vec_target_initial)
                        if (magnitude_type == "transfer_value_to_acheieve_magnitude")
                        else 0
                    )
                    ...
                    vec_transfer = (
                        ...
                        if (max(np.abs(vec_source_new - vec_transfer)) > 0)
                        else vec_transfer
                    )
                    ...
                    if magnitude_type == "baseline_scalar":
                        ...
                        arr_final = np.outer(
                            ...
                            + magnitude * (bounds[1] - arr_base[ind_tp_baseline, :]),
                        )
                ...
                arr_final = (
                    sf.vec_bounds(arr_final, bounds)
                    if (bounds is not None)
                    else arr_final
                )
...
def transformation_general_shift_fractions_from_modvars(
    ...
    dict_modvar_specs = dict(
        (k, v) for k, v in dict_modvar_specs.items() if (k in modvars) & sf.isnumber(v)
    )
    ...
    for x in modvars_target:
        ...
        (cats_all_init.append(set(cats)) if cats is not None else None)
    ...
    for region, df_in in df_in_grouped:
        ...
        for cat in cats_all:
            ...
            for modvar in modvars:
                ...
                append_q = cat in cats if not all_none else (cats is None)
