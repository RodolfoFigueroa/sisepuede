...
class Strategy:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if (self.table is not None) and ("df_input" not in kwargs):
            out = self.table

        else:
            out = self.function(*args, **kwargs)
            if "df_input" not in kwargs:
    ...
    def _initialize_table(
        ...
        **kwargs,
        ...
        table = self.function(**kwargs) if prebuild else None
...
class Strategies:
    ...
    def _initialize_base_input_database(
        ...
        regions = (
            ...
            if not sf.islistlike(regions)
                ...
                x
                for x in self.transformations.transformers.regions_manager.all_regions
                if x in regions
            ]
        )
        ...
        try:
            region_val = regions[0] if len(regions) > 0 else None
    ...
    def _initialize_file_structure(
        ...
        * self.file_struct
        ...
        """
        # set shortcut
        self.file_struct = self.transformations.transformers.file_struct
    ...
    def _initialize_transformations(
        ...
        * self.transformations
        ...
        self.model_attributes = (
            transformations.transformers.model_attributes
        )  # shortcut
    ...
    def _update_model_attributes(
        ...
        * self.transformations.transformers.model_attributes
        ...
        self.transformations.transformers.file_struct.model_attributes = (
            model_attributes
        )
    ...
    def get_attribute_table(
        ...
        df[self.field_baseline_strategy] = [
            int(x == baseline_id) for x in list(df[self.key_strategy])
        ]
    ...
    def get_strategy_definition_path(
        self,
        ...
        if not isinstance(path_specification, pathlib.Path):
            ...
            msg = "Invalid fn_strategy_definition specification in Strategies: input must be of type str or pathlib.Path"
            raise ValueError(msg)
    ...
    def build_strategies_to_templates(
        ...
        **kwargs
        ...
        if not sf.islistlike(strategies):
            ...
            strategies = [
                x for x in self.all_strategies_non_baseline if x in strategies_out
            ]
    ...
    def build_tornado_strategies(
        ...
        **kwargs,
        ...
        code_prependages_skip = (
            [] if not isinstance(code_prependages_skip, list) else code_prependages_skip
        )
        ...
        max_length_intersection = (
            np.inf if max_length_intersection is None else max_length_intersection
        )
        max_length_intersection = (
            0
    ...
    def build_whirlpool_strategies(
        self,
        ...
        **kwargs,
        ...
        ids = (
            list(range(max_id + 1, max_id + len(trans_specs) + 1)) if build_ids else ids
        )
