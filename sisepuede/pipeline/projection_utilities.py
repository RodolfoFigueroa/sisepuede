...
def repeat_value(
    ...
    for grouping, df in df_group:
        ...
        if n_proj == 0:
            df.append(
                df[df[field_time_period] <= tp_max_available].reset_index(drop=True),
            )
        ...
        df_rep = (
            df.sort_values(by=[field_time_period])
            .get(fields_data)
            .iloc[-n_lookback:]
        ...
        if method == "mean":
            df_rep = df_rep.rolling(n_lookback).mean().dropna()
        ...
        df_rep = df_rep[df_rep[field_time_period].isin(tps_proj)]
