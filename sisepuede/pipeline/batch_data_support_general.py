...
def exogenous_demands_to_sispeuede_ies(
    ...
    elasticity_bounds = (
        None if not isinstance(elasticity_bounds, tuple) else elasticity_bounds
    )
    elasticity_default = (
        1.0
        if not (
            isinstance(elasticity_default, float) or isinstance(elasticity_default, int)
        )
    ...
    max_dev_from_mean = (
        np.inf
        if not (
            isinstance(max_dev_from_mean, float) or isinstance(max_dev_from_mean, int)
        )
    ...
    sup_elast_magnitude = (
        np.inf
        if not (
            isinstance(sup_elast_magnitude, float)
            or isinstance(sup_elast_magnitude, int)
        )
    ...
    years_historical = [
        x for x in list(df_inputs[field_year].unique()) if (x < year_0_project)
    ]
    ...
    fields_dem = [
        x
        for x in model_attributes.build_variable_fields(modvar_demand)
    ...
    df_inputs_by_iso = (
        df_inputs[df_inputs[field_iso].isin(regions.all_isos)]
    ...
    for i, df in df_inputs_by_iso:
        ...
        if modvar_driver in model_socioeconomic.output_model_variables:
            if fill_missing_se is not None:
                ...
                fields_fill = [
                    x
                    for x in model_socioeconomic.required_variables
                    if (x not in df_inputs.columns)
                ]
                fields_dim_fill = [
                    x
                    for x in fields_fill
                    if x in model_socioeconomic.required_dimensions
                ]
        ...
        df_hist = (
            df[df[field_year].isin(years_historical)]
        ...
        for j in range(len(vec_means)):
            ...
            vec_cur = vec_cur[
                np.where((vec_cur > min(vec_cur)) & (vec_cur < max(vec_cur)))
            ]
            ...
            vec_means[j] = (
                float(sf.vec_bounds(mu, elasticity_bounds)) if not np.isnan(mu) else 1.0
            )
            vec_medians[j] = (
                float(sf.vec_bounds(med, elasticity_bounds))
                if not np.isnan(med)
                else 1.0
            )
