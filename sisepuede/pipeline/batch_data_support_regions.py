...
class region_solar:
    ...
    def get_datetime(
        ...
        *args,
        ...
        direction = (
            "utc_to_native"
            if (direction not in ["utc_to_native", "native_to_utc"])
            else direction
        )
    ...
    def build_solar_cf_seasonal_component_by_hour(
        self,
        ...
        attr_ts_group_1 = (
            attributes.get_other_attribute_table("ts_group_1")
            if (attributes is not None) and (attr_ts_group_1 is None)
            else attr_ts_group_1
        )
        attr_hour = (
            attributes.get_other_attribute_table("hour")
        ...
        for tsg in dict_tsg1_to_months.keys():
            ...
            for tp in attr_time_period.key_values:
                ...
                mo_base = (
                    n_months if (min([x % n_months for x in mos]) == 0) else min(mos)
                )
    ...
    if regions_valid is None:
        ...
        regions_valid = (
            ...
            if type_input == "region"
                ...
                x
                for x in attr_region.key_values
                if dict_region_to_iso.get(x) in (regions_valid)
            ]
        )
