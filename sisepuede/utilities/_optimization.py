...
class QAdjuster:
    ...
    def clean_and_reshape_solver_output(
        self,
        Q: np.ndarray,
        Q_solve: np.ndarray,
    ...
    def get_constraint_coeffs_error(
        self,
        matrix_0: np.ndarray,
        *,
        ...
        """
        error_type = (
            "additive" if error_type not in ["additive", "scalar"] else error_type
        )
        ...
        for k in range(n**2):
            ...
            sup_cur = (
                supremum if i not in inds_absorp else 1.0
            )  # max(supremum, matrix_0[i, j])
    ...
    def get_constraint_coeffs_max_area(
        self,
        x_0: np.ndarray,
    ...
    def get_constraint_coeffs_min_area(
        self,
        matrix_0: np.ndarray,
        x_0: np.ndarray,
        vector_bounds: np.ndarray,
    ...
    def get_constraint_coeffs_preserve_zeros(
        self,
        matrix_0: np.ndarray,
    ) -> np.ndarray:
    ...
    def get_constraint_coeffs_row_stochastic(
        self,
        matrix_0: np.ndarray,
    ) -> np.ndarray:
    ...
    def get_problem_components_objective(
        self,
        matrix_0: np.ndarray,
        x_0: np.ndarray,
        x_target: np.ndarray,
    ...
    def get_problem_components_constraints(
        self,
        matrix_0: np.ndarray,
        x_0: np.ndarray,
    ...
    def get_qp_component_vectors_euclidean_prevalence(
        self,
        x_0: np.ndarray,
        x_1: np.ndarray,
        ...
        vec_c = np.zeros(n2)
        M = np.zeros((n2, n2))
        weights = np.ones(n) if not isinstance(weights, np.ndarray) else weights
