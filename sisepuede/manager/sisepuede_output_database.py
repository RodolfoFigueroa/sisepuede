...
class SISEPUEDEOutputDatabase:
    ...
    def _check_dict_dimensional_keys(
        self,
        dict_dimensional_keys: Dict[str, str],
    ) -> None:
        ...
        * self.dict_dimensional_keys
        ...
        for key in self.required_dimensional_keys:
            ...
            if val is None:
                self._log(
                    f"Missing key dict_dimensional_keys: key {key} not found. Tables that rely on the {key} will not have index checking.",
                    type_log="warning",
                )
    ...
    def _initialize_output_database(
        ...
        **kwargs,
        ...
        try:
            ...
            self._log(
                "SISEPUEDEOutputDatabase successfully initialized IterativeDatabase.",
                type_log="info",
            )

        except Exception as e:
            self._log(
                f"Error in SISEPUEDEOutputDatabase initializing IterativeDatabase: {e}",
                type_log="error",
            )
    ...
    def _initialize_table_dicts(
        ...
        dict_derivative_table_functions = (
            {}
            if not isinstance(dict_derivative_table_functions, dict)
            else dict_derivative_table_functions
        )
        ...
        dict_all_tables.update(dict_return_idf_specs) if (
            dict_return_idf_specs is not None
        ) else None
        self.all_tables = sorted(list(dict_all_tables.keys()))
        self.dict_all_tables = dict_all_tables
        self.dict_derivative_table_functions = (
            dict_return_functions if (dict_return_functions is not None) else None
        )
    ...
    def validate_derivative_table_function(
        self,
        src_table: str,
        function: Callable,
        ...
        dict_source_tables = (
            self.dict_all_tables
            if not isinstance(dict_source_tables, dict)
            else dict_source_tables
        )
    ...
    def validate_derivative_tables(
        self,
        ...
        for x in dict_derivative_table_functions.keys():
            ...
            if table_src is not None:
                ...
                if x in dict_all_tables.keys():
                    self._log(
                        f"Derivative table name '{x}' already found in list of SISEPUEDE tables. Trying to rename...",
                        type_log="warning",
                    )
                    ...
                    if key_try in dict_all_tables.keys():
                        ...
                        if append_integer_to_field:
                            ...
                            while (key_try in dict_all_tables.keys()) and (
                                i < max_iter
                            ):
                                i += 1
                                key_try = f"{key_try}_{i}"
                        ...
                        self._log(
                            f"Dertivative table '{x}' successfully renamed to '{key_try}'",
                            type_log="info",
                        )

                fields_index = self.dict_all_tables.get(table_src)
                fields_index = (
                    fields_index.get(str_fields_index_param)
                    if isinstance(fields_index, dict)
                    else None
                )
