...
class SISEPUEDEExperimentalManager:
    ...
    def _initialize_archival_settings(
        self,
    ) -> None:
        ...
        * self.
        ...
        """
        self.archive_table_name_experimental_configuration = (
    ...
    def _initialize_attribute_design(
        ...
        * self.key_design
        ...
        if not isinstance(attribute_design, AttributeTable):
            ...
            self._log(
                f"Invalid type '{tp}' in specification of attribute_design: attribute_design should be an AttributeTable.",
                type_log="error",
            )
    ...
    def _initialize_baseline_database(
        self,
        fp_templates: str,
    ...
    def _initialize_future_trajectories(
        self,
        fp_exogenous_xl_type_for_variable_specifcations: Union[str, None] = None,
        **kwargs,
    ) -> None:
        ...
        dfg = self.base_input_database.database.groupby([self.field_region])
        ...
        for region, df in dfg:
            ...
            try:
                ...
                sampling_units_varying_x = [
                    k
                    for k, v in future_trajectories_cur.dict_sampling_units.items()
                    if v.x_varies
                ]
                ...
                self._log(
                    f"\tFutureTrajectories for '{region_print}' complete.",
                    type_log="info",
                )
        ...
        * self.regions
        ...
        for region in self.regions:
            ...
            try:
                ...
                self._log(
                    f"\tLHSDesign for region '{region_print}' complete.",
                    type_log="info",
                )

            except Exception as e:
                self._log(
                    f"Error initializing LHSDesign for region '{region_print}' -- {e}.",
                    type_log="error",
                )
                self._log(
                    f"Dropping region '{region_print}' due to error in LHSDesign initialization.",
                    type_log="warning",
                )
                drop_regions.append(region)
    ...
    def _initialize_other_properties(
        ...
        * self.random_seed
        ...
        self.sort_ordered_dimensions_of_analysis = (
            self.model_attributes.sort_ordered_dimensions_of_analysis
        )
    ...
    def _initialize_primary_keys_index(
        self,
    ) -> None:
        ...
        * self.primary_key_database
        """
        self._log(
            f"Generating primary keys (values of {self.key_primary})...",
            type_log="info",
        )
        ...
        all_futures += (
            self.vector_lhs_key_values
            if (self.vector_lhs_key_values is not None)
            else []
        )
