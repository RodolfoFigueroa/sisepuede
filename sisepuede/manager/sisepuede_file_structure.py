...
class SISEPUEDEFileStructure:
    ...
    def _check_config(
        ...
        * self.fn_config
        ...
        if isinstance(fn_config, str):
            self.fn_config = (
                fn_config if fn_config.endswith(".config") else self.fn_config
            )
    ...
    def _check_required_directories(
        ...
        * self.fp_config
        ...
        self.dir_attribute_tables = (
            os.path.join(self.dir_proj, "attributes")
            if (self.dir_proj is not None)
            else ""
        )
        ...
        self.dir_ref_nemo = (
            os.path.join(self.dir_ref, "nemo_mod") if (self.dir_ref is not None) else ""
        )
        ...
        if count_errors > 0:
            self._log(
                f"There were {count_errors} errors initializing the SISEPUEDE directory structure:{msg_error_dirs}",
                type_log="error",
            )
            raise RuntimeError(
                "SISEPUEDE unable to initialize file directories. Check the log for more information.",
            )
        self._log(
            "Verification of SISEPUEDE directory structure completed successfully with 0 errors.",
            type_log="info",
            warn_if_none=False,
        )
    ...
    def _check_ingestion(
        ...
        set_default |= (
            (not os.path.exists(dir_ingestion)) if not set_default else set_default
        )
    ...
    def _check_optional_directories(
        self,
        initialize_directories: bool = True,
    ) -> None:
    ...
    def _check_nemomod_reference_file_paths(
        self,
    ) -> None:
        ...
        * self.required_reference_tables_nemomod
        ...
        if (self.dir_ref_nemo is not None) and (self.dir_jl is not None):
            ...
            self.required_reference_tables_nemomod = (
                model_enerprod.required_reference_tables
            )

            # initialize dictionary of file paths
            dict_nemomod_reference_tables_to_fp_csv = dict(
                zip(
                    self.required_reference_tables_nemomod,
        ...
        if msg_error != "":
            ...
            self._log(
                "NemoMod reference file checks completed successfully.",
                type_log="info",
                warn_if_none=False,
            )
    ...
    def _initialize_file_path_defaults(
        self,
        initialize_directories: bool = True,
    ) -> None:
    ...
    def try_restore_model_attributes_from_pickle(
        self,
        ...
        if (not os.path.exists(fp_pkl)) | (fp_pkl is None):
            self._log(
                f"Path to model_attributes pickle '{fp_pkl}' not found. The session cannot be loaded.",
                type_log="error",
            )
            return None
        ...
        try:
            ...
            self._log(
                f"Error trying to load model_attributes from pickle at '{fp_pkl}': {e}",
                type_log="error",
            )
    ...
    def _write_model_attributes_to_pickle(
        self,
        model_attributes: ma.ModelAttributes,
        fp_pkl: Union[str, None] = None,
        key_model_attributes: Union[str, None] = None,
        ...
        if os.path.exists(fp_pkl):
            self._log(
                f"Path to model_attributes pickle '{fp_pkl}' already exists. The file will not be overwritten.",
                type_log="error",
            )
            return
        ...
        try:
            ...
            self._log(
                f"Successfully archived self.model_attributes to pickle at '{fp_pkl}'",
                type_log="info",
            )

        except Exception as e:
            self._log(
                f"Error trying to write self.model_attributes to pickle at '{fp_pkl}': {e}",
                type_log="error",
            )
