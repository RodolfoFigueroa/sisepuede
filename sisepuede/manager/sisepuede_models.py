...
class SISEPUEDEModels:
    ...
    def _initialize_path_julia(
        ...
        if isinstance(fp_julia, str):
            if os.path.exists(fp_julia):
                ...
                self._log(
                    f"Set Julia directory for modules and environment to '{self.fp_julia}'.",
                    type_log="info",
                )
            else:
                self.allow_electricity_run = False
                self._log(
                    f"Invalid path '{fp_julia}' specified for Julia reference modules and environment: the path does not exist. Setting self.allow_electricity_run = False.",
                    type_log="error",
                )

    def _initialize_path_nemomod_reference(
        self,
        allow_electricity_run: bool,
        fp_nemomod_reference_files: Union[str, None],
    ) -> None:
    ...
    def _initialize_path_nemomod_sql(
        self,
        fp_nemomod_temp_sqlite_db: Union[str, None],
    ) -> None:
        """Initialize the path to the NemoMod SQL database used to execute runs.
        ...
        * self.fp_nemomod_temp_sqlite_db
        ...
        if isinstance(fp_nemomod_temp_sqlite_db, str):
            try_endings = [
                fp_nemomod_temp_sqlite_db.endswith(x) for x in valid_extensions
            ]
        ...
        os.remove(self.fp_nemomod_temp_sqlite_db) if os.path.exists(
            self.fp_nemomod_temp_sqlite_db,
        ) else None
    ...
    def check_model_results(
        ...
        return_none |= not (
            ...
            if not isinstance(fields_index, str)
                ...
                fields_index.lower()
                in ["emissions_output", "emissions_output_subsector_aggregate"]
            )
        )
        ...
        fields_check = (
            ...
            if not sf.islistlike(fields_check)
                ...
                x
                for x in fields_check
                if (x not in fields_index) and (x in df_results.columns)
            ]
        )
        fields_check = (
            [
                x
                for x in fields_check
                if x in self.model_attributes.all_variable_fields_output
            ]
            if output_only
            else fields_check
        )
    ...
    def project(
        ...
        **kwargs :
        ...
        """
        df_return = []
        models_run = self.model_attributes.get_sector_list_from_projection_input(
            models_run,
        )
        ...
        if time_periods_run is not None:
            time_periods_run = (
                None if (len(time_periods_run) == 0) else time_periods_run
            )

        df_input_data = (
            (
                df_input_data[
                    df_input_data[self.model_attributes.dim_time_period].isin(
                        time_periods_run,
                    )
        ...
        if "AFOLU" in models_run:
            ...
            try:
                ...
                self._log("AFOLU model run successfully completed", type_log="info")
        ...
        if "IPPU" in models_run:
            ...
            try:
                ...
                self._log("IPPU model run successfully completed", type_log="info")
        ...
        if "Energy" in models_run:
            ...
            try:
                ...
                self._log(
                    "EnergyConsumption without Fugitive Emissions model run successfully completed",
                    type_log="info",
                )
        ...
        if (
            ("Energy" in models_run)
            and include_electricity_in_energy
            and self.allow_electricity_run
        ):
            self._log(
            ...
            if run_integrated and set(["Circular Economy", "AFOLU"]).issubset(
                set(models_run),
            ):
                df_input_data = self.model_attributes.transfer_df_variables(
                    df_input_data,
            ...
            engine = sqlalchemy.create_engine(
                f"sqlite:///{self.fp_nemomod_temp_sqlite_db}",
            )
            try:
                ...
                self._log(
                    "EnergyProduction model run successfully completed",
                    type_log="info",
                )
        ...
        if "Energy" in models_run:
            ...
            try:
                ...
                self._log(
                    "Fugitive Emissions from Energy model run successfully completed",
                    type_log="info",
                )
        ...
        if len(df_return) > 0:
            ...
            try:
                ...
                self._log(
                    "Socioeconomic outputs successfully appended.",
                    type_log="info",
                )
        ...
        if check_results:
            ...
            df_return = df_return if return_df else None
