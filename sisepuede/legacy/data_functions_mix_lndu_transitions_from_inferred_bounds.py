...
class MixedLNDUTransitionFromBounds:
    ...
    def _initialize_dicts(
        ...
        self.df_all_years = (
            self.df_all_transitions[[self.field_year]]
            .drop_duplicates()
            .sort_values(
                by=[self.field_year],
            )
        ...
        self.filter_quality_ranking = (
            ...
            if (
                isinstance(filter_quality_ranking, int)
                or (filter_quality_ranking is None)
            )
            else None
        )
        ...
        for region in self.all_regions:
            ...
            for bound in [self.bound_0, self.bound_1]:
                ...
                if self.filter_quality_ranking is not None:
                    if self.filter_quality_ranking < 0:
                        df_all_cur = df_all_cur[
                            df_all_cur[self.field_design_quality_rank]
                            == min(df_all_cur[self.field_design_quality_rank])
                        ]
                    else:
                        df_all_cur = df_all_cur[
                            df_all_cur[self.field_design_quality_rank]
                ...
                df_all_cur = (
                    self.check_row_sums(df_all_cur, eps)
                    if (len(df_all_cur) > 0)
                    else None
                )
    ...
    def _initialize_fields(
        ...
        * self.dict_fields_mix_to_fields_out
        ...
        self.fields_mix = [
            x
            for x in self.df_all_transitions.columns
            if (x in self.df_mean_transitions.columns)
            and (
                x
                not in [
                    self.field_region,
                    self.field_year,
                    self.field_optimization_bound,
                ]
            )
        ]
    ...
    def _initialize_tables(
        ...
        * self.fp_mean_transitions
        ...
        self.fp_all_transitions = (
            ...
            if (fp_all_transitions is None)
            else fp_all_transitions
        )
        self.fp_mean_transitions = (
            sa.fp_csv_transition_probability_estimation_mean
            if (fp_mean_transitions is None)
            else fp_mean_transitions
        )
        ...
        self.df_best_design_quality_rankings = (
            self.df_all_transitions.groupby(fields_grp)
            .agg(dict_agg)
    ...
    def mix_transitions(
        ...
        dict_by_region = (
            self.dict_all_transitions.get(region)
            if (transition_type == "annual")
            else self.dict_mean_transitions.get(region)
        )
        ...
        df_out = (
            pd.concat([self.df_all_years, df_out], axis=1).reset_index(drop=True)
            if (transition_type == "annual")
            else df_out
        )
