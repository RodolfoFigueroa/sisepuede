...
class ShellTemplate:
    ...
    def _initialize_replacement_matchstrings(
        self,
        char_esc: str = "$$",
    ) -> None:
        ...
        char_esc_regex = char_esc.replace("$", r"\$")
    ...
    def get_template_matchstring(
        ...
        """
        char_esc = self.char_esc if not isinstance(char_esc, str) else char_esc
...
class AWSManager:
    ...
    def build_s3_query_output_file_names(
        self,
        ...
        appendage_table_name = (
            "" if not isinstance(appendage_table_name, str) else appendage_table_name
        )

        # build dictionary of file base names
        keys_modify = ["input", "output"]
        dict_fbn = dict((x, x) for x in valid_types)
    ...
    def build_table_name_to_type_dicts(
        self,
    ) -> Tuple[Dict, Dict]:
    ...
    def get_sisepuede_relative_paths(
        self,
        path_type: str,
    ) -> Union[str, None]:
    ...
    def get_user_data_subtemplate_partitions(
        self,
        template: str,
        delim_end: str = "END",
        delim_newline: str = "\n",
    ...
    def _initialize_attributes(
        ...
        * self.sisepuede_database
        ...
        regex_config_experiment_components = re.compile(r"part_(\d*$)")
    ...
    def _initialize_aws(
        ...
        # check profile
        profile_name = (
            default_profile_name if not isinstance(profile_name, str) else profile_name
        )
        ...
        command_aws = "aws" if not isinstance(command_aws, str) else command_aws
    ...
    def _initialize_aws_properties(
        self,
        partitions_ordered: Union[List[str], None] = None,
    ) -> None:
    ...
    def _initialize_docker_properties(
        self,
    ) -> None:
        """Use the configuration file to set some key shared properties for AWS.
            Sets the following properties:
    ...
    def _initialize_shell_script_environment_vars(
        self,
    ) -> None:
    ...
    def _initialize_table_attributes(
        self,
    ) -> None:
        ...
        for key, table in dict_input_output_to_table_names.items():
            ...
            keys_table_index_ordered_cur = [
                x
                for x in self.model_attributes.sort_ordered_dimensions_of_analysis
    ...
    def _initialize_template_matchstrs(
        self,
    ) -> None:
    ...
    def build_docker_shell_flags(
        ...
        for k, v in dict_subset.items():
            v_out = delim_elems.join([str(x) for x in v]) if sf.islistlike(v) else v
        ...
        (flags.append(f"--{flag} {db_type}") if isinstance(flag, str) else None)

        # add in id type
        id_str = self.file_struct.id
        flag = self.dict_dims_to_docker_flags.get(self.flag_key_id)
        (flags.append(f'--{flag} "{id_str}"') if isinstance(flag, str) else None)
        ...
        (flags.append(f"--{flag}") if isinstance(flag, str) & save_inputs else None)
        ...
        (flags.append(f"--{flag}") if isinstance(flag, str) & try_exog_xl else None)

        flags = delim_args.join(flags)

        return flags

    def build_docker_shell(
        self,
        dict_subset: Dict[str, List[int]],
        **kwargs,
    ) -> str:
    ...
    def build_keyvals_echo_string(
        ...
        str_out = list(values).copy() if sf.islistlike(values) else []

        (str_out.insert(0, keyname) if isinstance(keyname, str) else None)

        (str_out.append("") if len(str_out) > 0 else None)
    ...
    def build_user_data_str_cd_home_dir(
        self,
    ) -> str:
    ...
    def build_user_data_str_copy_to_s3(
        self,
        ...
        bucket = bucket if self.validate_bucket(bucket) else None
        ...
        comm = (
            f"{self.command_aws} s3 cp '{self.fp_instance_instance_info}' '{fp_target}'"
        )
        lines_out.append(comm)
        ...
        comm = (
            f'{self.command_aws} s3 cp "{self.fp_instance_sisepuede_log}" "{fp_target}"'
        )
        lines_out.append(comm)
    ...
    def build_user_data_str_dimensional_reads(
        self,
        dict_subset: Dict[str, List[int]],
        delim_newline: str = "\n",
        output_type: str = "echo",
        ...
        delim = delim_newline if output_type == "echo" else " "
    ...
    def build_user_data_str_docker_pull(
        self,
        delim_imgname: str = "/",
        delim_newline: str = "\n",
    ) -> str:
        ...
        if use_ecr:
            ...
            comm_ecr = (
                f"{self.command_aws} ecr get-login-password --region {aws_region}"
            )
            comm_ecr = (
                f"{comm_ecr} | docker login --username AWS --password-stdin {std_in}"
            )
    ...
    def build_user_data_str_docker_run(
        self,
    ) -> str:
    ...
    def build_user_data_str_docker_shell(
        self,
        dict_dimensional_subset: Dict[str, List[int]],
        **kwargs,
    ) -> str:
    ...
    def build_user_data_str_echo_from_keyvals(
        self,
        keyname: Union[str, None],
        values: List[Any],
        fbn_values: str,
    ...
    def build_user_data_str_instance_metadata(
        self,
        dict_subset: Dict[str, List[int]],
        ...
        command_get_instance_id = (
            f"{env_var_assign}=$(curl {self.aws_url_instance_id_metadata})"
        )
    ...
    def build_user_data_str_mkdir_instance_mount(
        self,
        permissions: str = "777",
    ) -> str:
        """Build the docker pull command
    ...
    def build_user_data_str_screen_wrapper(
        self,
        ...
        fp_shell_script = os.path.join(self.dir_instance_home, f"{key}.sh")

        # get the screen name
        screen_name = self.config.get("aws_ec2.screen_name")
        screen_name = (
            default_screen_name if not isinstance(screen_name, str) else screen_name
        )
    ...
    def build_user_data_str_terminate_instance(
        self,
    ) -> str:
    ...
    def build_ec2_iam_instance_profile_regex(
        self,
        delim_name: str = "/",
    ...
    def format_for_ec2_iam_instance_profile(
        self,
        ...
        dict_out = {}
        (dict_out.update({key_arn: instance_iam_role}) if return_arn else None)
        (dict_out.update({key_name: list_components[1]}) if return_name else None)

        return dict_out

    def format_for_ec2_tags(
        self,
    ...
    def format_table_name_for_athena(
        self,
        table_name: str,
    ) -> str:
    ...
    def get_buckets(
        ...
        client_s3 = self.client_s3 if client_s3 is None else client_s3
    ...
    def get_path_instance_metadata(
        self,
        dir_out: str,
    ) -> str:
    ...
    def get_path_instance_output_db(
        self,
        ...
        """
        dir_instance_out = (
            self.dir_instance_out if dir_instance_out is None else dir_instance_out
        )
    ...
    def get_path_instance_output_run_package(
        self,
        dir_instance_out: Union[str, None] = None,
    ) -> str:
    ...
    def get_time_hash_for_table(
        ...
        dict_repl = dict((x, "") for x in ["-", ";", ":", ".", "T", id_prependage])
    ...
    def get_regions(
        ...
        regions = regions_config if regions is None else regions

        regions = regions.split(delim) if isinstance(regions, str) else regions
    ...
    def get_s3_bucket_and_key_from_address(
        self,
        address: str,
        sep: str = "/",
        ...
        key = tup[1] if len(tup) > 1 else None

        out = (self.s3_bucket, key)

        return out

    def get_s3_path_athena_output(
        self,
        return_type: str,
    ) -> Union[str, None]:
    ...
    def get_s3_path_athena_table_file(
        self,
        table_name: str,
        ...
        # get basename
        base_name = (
            table_name if not isinstance(index, int) else f"{table_name}_{index}"
        )
        ...
        if isinstance(dict_partitions, dict) & (
            self.athena_partitions_ordered is not None
        ):
    ...
    def get_s3_path_instance_metadata(
        self,
    ) -> Union[str, None]:
    ...
    def get_tables_to_copy_to_s3(
        ...
        """
        tables = self.config.get("aws_s3.tables_copy") if tables is None else tables

        tables = tables.split(delim) if isinstance(tables, str) else tables

        tables = tables if sf.islistlike(tables) else None
    ...
    def launch_instance(
        ...
        ami = self.config.get("aws_ec2.image") if not isinstance(ami, str) else ami
        ...
        tags = (
            self.config.get("aws_general.tag") if not isinstance(tags, Dict) else tags
        )
        (tags.update({"Name": name}) if isinstance(name, str) else None)
    ...
    def build_athena_sql_query_component_primary_keys_all(
        self,
        delim: str = ",",
        ...
        n_regions = (
            len(self.get_regions()) if not isinstance(n_regions, int) else n_regions
        )
    ...
    def build_athena_query_create_table(
        self,
        table_name: str,
        schema: str,
        ...
        query_create_table = (
            f"CREATE EXTERNAL TABLE IF NOT EXISTS {database_name}.{table_name_out}"
        )
        ...
        str_partition = (
            f"(\n{str_partition[0]}\n)" if str_partition is not None else str_partition
        )

        # add the string for row formatting
        str_row_format = """DELIMITED
    ...
    def build_athena_query_repair_table(
        self,
        table_name: str,
    ) -> str:
    ...
    def build_athena_sql_table_schema_for_inputs_outputs(
        self,
        ...
        fields_data_integer = (
            [] if not sf.islistlike(fields_data_integer) else fields_data_integer
        )
        ...
        for i, field in enumerate(fields):
            ...
            if dtype_sql is None:
                dtype_sql = "int64" if field in fields_data_integer else "float64"
            ...
            field_name = (
                ...
                if (field in self.athena_partitions_ordered)
                & isinstance(partition_field_appendage, str)
                else field
            )
    ...
    def build_athena_sql_query_for_retrieval(
        self,
        delim: str = ",",
        ...
        fields_ind = [
            x
            for x in fields_ind
            if x in self.dict_keys_table_name_to_index_ordered.get(table_name_output)
        ]
    ...
    def build_athena_sql_query_for_retrieval_by_io(
        self,
        table_type: str,
    ...
    def build_experiment(
        ...
        return_type = (
            return_type if return_type in ["list", "indexing_data_frame"] else "list"
        )
        set_primaries = set({})
    ...
    def check_queries_success(
        ...
        # check execution ids
        execution_ids = [execution_ids] if exec_ids_entered_as_str else execution_ids
        execution_ids = execution_ids if sf.islistlike(execution_ids) else None
        ...
        out = dict((x, (dict_states.get(x) == state_succeeded)) for x in execution_ids)
        out = list(out.values())[0] if exec_ids_entered_as_str else out

        return out

    def execute_create_table_athena_query(
        self,
        fields_data_integer: Union[List[str], None] = None,
        ...
        **kwargs,
        ...
        s3p_out = self.s3p_athena_queries if not isinstance(s3p_out, str) else s3p_out
    ...
    def exec_query(
        ...
        """
        log_msg_supplement = (
            "" if not isinstance(log_msg_supplement, str) else log_msg_supplement
        )
        response = None
    ...
    def get_design_splits_by_launch_index(
        self,
    ...
    def get_design_splits_by_launch_index_from_run_information(
        self,
    ...
    def get_experimental_dictionary(
        self,
        delim: str = ",",
        dict_experimental_components: Union[
            Dict[str, Union[Dict[str, List[int]], List[int]]],
        ...
        keys_parts = [
            x
            for x in list(dict_experimental_components.keys())
        ...
        dict_experimental_components = dict_out if len(dict_out) > 0 else None
    ...
    def get_query_state(
        ...
        client = self.client_athena if client is None else client
        ...
        query_exec = (
            response.get("QueryExecution") if isinstance(response, dict) else None
        )
        query_exec = query_exec.get("Status") if isinstance(query_exec, dict) else None
        state_cur = query_exec.get("State") if isinstance(query_exec, dict) else None
    ...
    def get_random_seeds(
        ...
        uniform_seed_q = uniform_seed_q if isinstance(uniform_seed_q, bool) else False
        ...
        **kwargs,
        ...
        dims = (
            [
                x
                for x in dims
                if f"dim_{x}" in self.model_attributes.dict_attributes.keys()
            ]
    ...
    def _write_attribute_primary(
        ...
        **kwargs,
        ...
        table_dim = (
            table_dim.replace("_ID", "") if strip_ids_from_table_names else table_dim
        )
