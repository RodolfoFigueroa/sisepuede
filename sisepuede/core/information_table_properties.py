...
def build_emissions_information_table(
    ...
    for gas in dict_gas_to_emision_modvars.keys():
        ...
        for modvar in modvars:
            ...
            pycats_primary = [("" if (x == "") else pycat_primary) for x in cats]
    ...
    df_out = pd.concat(df_out, axis=0).sort_values(by=fields_ord).reset_index(drop=True)
...
def build_variable_information_table(
    ...
    modvars = (
        model_attributes.all_variables if not sf.islistlike(modvars) else list(modvars)
    )
