...
class ModelAttributes:
    ...
    def iat_support_get_attribute_group(
        self,
        fn_attribute: str,
    ...
    def _iat_support_clean_subsector_table(
        self,
    ...
    def _iat_support_update_grouped_attributes(
        self,
        fp_attribute: str,
        attribute_groups_ordered: List[str],
        dict_attribute_group_to_regex: Dict[str, re.Pattern],
        ...
        regex = (
            dict_attribute_group_to_regex.get(group) if (group is not None) else None
        )
    ...
    def _initialize_attribute_tables(
        ...
        dict_attribute_group_to_regex = dict(
            ...
            for x in attribute_groups
            if (x != "other")
        )
        dict_attribute_group_to_regex.update(
            {attribute_group_protected_other: re.compile("attribute_(.*).csv")},
        )
        ...
        dict_attributes = dict((x, {}) for x in dict_attribute_group_to_regex)
    ...
    def _initialize_other_attributes(
        self,
    ) -> None:
    ...
    def extract_model_variable(
        ...
        fill_value = all_cats_missing_val if sf.isnumber(all_cats_missing_val) else None
    ...
    def filter_keys_by_attribute(
        ...
        subsector_extract_key = (
            subsector_extract_key
            if isinstance(subsector_extract_key, str)
            & (subsector_extract_key in attr.table.columns)
            else attr.key
        )
    ...
    def get_all_subsector_emission_total_fields(
        ...
        return_type = "list" if (return_type not in valid_types) else return_type
        ...
        if return_type in ["dict", "dict_abv"]:
            out = dict(zip(subsectors_emission, out, strict=False))
        elif return_type in ["dict_inv", "dict_inv_abv"]:
            out = dict(zip(out, subsectors_emission, strict=False))
    ...
    def get_dimensional_attribute_table(
        self,  # FIXED
        dimension: str,
    ...
    def get_sector_attribute_table(
        self,  # FIXED
    ) -> Union[AttributeTable, None]:
    ...
    def get_sector_emission_total_fields(
        self,
        sector: str,
    ) -> Union[List[str], None]:
        ...
        fields_subsector_total = [
            self.get_subsector_emission_total_field(x) for x in subsecs
        ]
    ...
    def get_subsector_attribute_table(
        self,  # FIXED
    ) -> Union[AttributeTable, None]:
    ...
    def get_subsector_color_map(
        ...
        dict_subsec_field_to_color = dict(
            (k, dict_subsec_abv_to_color.get(v))
            for k, v in dict_subsec_field_to_abv.items()
        )
    ...
    def get_subsector_variables(
        ...
        for k, v in self.dict_variables.items():
            ...
            append_q = (
                (v_type == var_type.lower()) if isinstance(var_type, str) else True
            )
    ...
    def merge_array_var_partial_cat_to_array_all_cats(
        ...
        if modvar is not None:
            ...
            missing_vals = (
                def_missing if not sf.isnumber(missing_vals) else missing_vals
            )
    ...
    def overwrite_variable_from_mix(
        self,
    ...
    def get_energy_power_swap(
        ...
        time_period = (
            "annualized"
            if (time_period not in ["annualized", "hourly"])
            else time_period
        )
    ...
    def get_variable(
        ...
        """
        out = (
            modvar if mv.is_model_variable(modvar) else self.dict_variables.get(modvar)
        )
    ...
    def get_scalar(
        ...
        if return_type == "area":
            ...
            scalar_out *= (
                self.get_area_equivalent(area.lower()) if isinstance(area, str) else 1
            )
        ...
        if return_type in [
            ...
            scalar_out *= (
                self.get_mass_equivalent(mass.lower()) if isinstance(mass, str) else 1
            )
            ...
            for var in vars_subsec:
                ...
                if (var_type == "output") and gas:
                    total_emission_modvars_by_gas = (
                        self.dict_gas_to_total_emission_variables.get(gas)
                    )
    ...
    def exchange_year_time_period(
        self,  # REVIEW: CALLS SHOULD BE SWAPPED TO USE sc.TimePeriods
        df_in: pd.DataFrame,
        field_year_new: str,
    ...
    def build_emission_total_fields_info_df(
        self,
    ) -> pd.DataFrame:
    ...
    def build_modvar_correspondence_dictionary(
        self,
        modvar_key: str,
        modvar_value: str,
    ) -> Dict[str, str]:
        ...
        **kwargs,
        ...
        for sector in sectors_build:
            ...
            for subsector in subsectors:
                ...
                vars_cur = sum(
                    [
                        self.dict_model_variables_to_variable_fields.get(x)
        ...
        if isinstance(df_trajgroup, pd.DataFrame):
            ...
            if set([field_variable_field, field_variable_trajectory_group]).issubset(
                ...
                if field_variable_trajectory_group in df_out.columns:
                    ...
                    df_out_keep = df_out[
                        ~df_out[field_variable_field].isin(vars_to_assign)
                    ]
    ...
    def build_target_variable_fields_from_source_variable_categories(
        self,  # FIXED
    ...
    def build_variable_fields(
        ...
        **kwargs,
        ...
        # check variable type
        variable_type = (
            variable_type.lower() if isinstance(variable_type, str) else variable_type
        )
        variable_type = (
            None if variable_type not in ["input", "output"] else variable_type
        )
    ...
    def get_input_output_fields(
        ...
        subsectors_io = (
            subsectors_io if sf.islistlike(subsectors_io) else [subsectors_io]
        )
    ...
    def get_multivariables_with_bounded_sum_by_category(
        ...
        dict_arrs = {}
        init_q = True
        modvars = [modvars] if not sf.islistlike(modvars) else modvars
    ...
    def get_simplex_group_specification(
        ...
        if len(modvars_to_check) == 1:
            dict_out.update(
                dict(
                    (x, ind_base)
                    for x in self.build_variable_fields(modvars_to_check[0])
                ),
            )
