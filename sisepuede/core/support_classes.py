...
class Regions:
    ...
    def _initialize_region_properties(
        self,
        model_attributes: ModelAttributes,
        field_year: str = "year",
        regex_region_groups: Union[re.Pattern, None] = re.compile("(.*)_region$"),
    ...
    def add_region_or_iso_field(
        ...
        **kwargs
        ...
        """
        field_iso = self.field_iso if not isinstance(field_iso, str) else field_iso
        field_region = self.key if not isinstance(field_region, str) else field_region
    ...
    def aggregate_df_by_region_group(
        self,
        df_in: pd.DataFrame,
        region_grouping: str,
        ...
        if field_merge in dict_agg_pass:
    ...
    def convert_region_codes(
        ...
        df_merge = (
            self.attributes.table[[field_input, field_output]]
            .copy()
            .rename(
    ...
    def fill_missing_regions(
        ...
        if region_spec == _RETURN_TYPE_ISO:
            ...
            field_region = (
                self.field_iso_numeric if field_region is None else field_region
            )
        ...
        regions_missing = [
            x for x in regions_space if x not in df[field_region].unique()
        ]
    ...
    def fill_missing_regions_using_grouping_aggregate(
        self,
        df: pd.DataFrame,
        region_grouping: str,
        field_region: Union[str, None],
        ...
        all_group_vals = set(
            [self.get_region_group(region, region_grouping) for region in regions_fill],
        )
        ...
        dict_rg_to_regions = dict_rg_to_regions[
            dict_rg_to_regions[field_rg].isin(all_group_vals)
        ][[self.key, field_rg]]
    ...
    def get_closest_region(
        ...
        type_return = (
            _RETURN_TYPE_REGION
            if (type_return not in self.valid_return_types)
            else type_return
        )
        type_input = (
            _RETURN_TYPE_REGION
            if (type_input not in self.valid_return_types)
            else type_input
        )
        ...
        df_regions = (
            attr_region.table[attr_region.table[attr_region.key].isin(regions_valid)]
            .copy()
            .reset_index(drop=True)
        )
        ...
        vec_dists = np.array(
            df_regions[[self.field_lat, self.field_lon]].apply(f, raw=True, axis=1),
        )
        ...
        if len(valid_dists) > 0:
            ...
            out = list(df_regions[attr_region.key])[w[0]] if len(w) > 0 else None
    ...
    def get_hierarchical_regions_field(
        self,
        df: pd.DataFrame,
    ...
    def data_func_iea_get_isos_from_countries(
        self,
        ...
        """
        field_country = (
            self.field_iea_country if (field_country is None) else field_country
        )
        vec_iso = (
            list(df_in[field_country])
            if isinstance(df_in, pd.DataFrame)
            else ([df_in] if isinstance(df_in, str) else df_in)
        )
    ...
    def data_func_try_isos_from_countries(
        self,
        df_in: Union[pd.DataFrame, List, np.ndarray, str],
        field_country: Union[str, None] = None,
        ...
        missing_iso_flag = (
            str(missing_iso_flag) if missing_iso_flag is not None else None
        )
        ...
        for i, region_base in enumerate(vec_iso):
            ...
            # set a hierarchy
            region_full = region if region in self.dict_region_to_iso.keys() else None
            region_full = (
            ...
            flag = region_base if (missing_iso_flag is None) else missing_iso_flag
            flag = (
                region.upper()
...
class TimePeriods:
    ...
    def _initialize_time_properties(
        self,
        model_attributes: ModelAttributes,
        field_year: str = "year",
    ) -> None:
    ...
    def tp_to_year(
        ...
        time_period = (
            ...
            if isinstance(time_period, int)
                ...
                int(np.round(time_period)) if isinstance(time_period, float) else None
            )
        )
    ...
    def year_to_tp(
        ...
        year = (
            year
            if sf.isnumber(year, integer=True)
            else (int(np.round(year)) if sf.isnumber(year) else None)
        )
...
class YAMLConfiguration:
    ...
    def get(
        ...
        value = (
            value if (key == keys_nested[-1]) & (value is not None) else return_on_none
        )
