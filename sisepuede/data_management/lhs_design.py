...
class LHSDesign:
    ...
    def _set_ignore_trial_flag(
        self,
        ignore_trial_flag: float,
    ) -> None:
        self.ignore_trial_flag = (
            float(min(ignore_trial_flag, -1.0))
            if (
                isinstance(ignore_trial_flag, float)
                or isinstance(ignore_trial_flag, int)
            )
            else -1.0
        )

    def _set_lhs_parameters(
        self,
        fields_factors_l: Union[List[str], List[int], None] = None,
        fields_factors_x: Union[List[str], List[int], None] = None,
        n_factors_x: Union[int, None] = None,
        ...
        if (
            (self.n_factors_l is None) and (n_factors_l is not None)
        ) or n_factors_l is not None:
            ...
            if self.n_factors_l is not None:
                ...
                if isinstance(fields_factors_l, list):
                    if len(fields_factors_l) == self.n_factors_l:
                        ...
                        self._log(
                            "LHSDesign.fields_factors_l reset successful.",
                            type_log="info",
                        )
                    else:
                        self._log(
        ...
        if (
            (self.n_factors_x is None) and (n_factors_x is not None)
        ) or n_factors_x is not None:
            ...
            if self.n_factors_x is not None:
                ...
                if isinstance(fields_factors_x, list):
                    if len(fields_factors_x) == self.n_factors_x:
                        ...
                        self._log(
                            "LHSDesign.fields_factors_x reset successful.",
                            type_log="info",
                        )
                    else:
                        self._log(
        ...
        self.vector_lhs_key_values = (
            list(range(1, self.n_trials + 1)) if (self.n_trials is not None) else None
        )
    ...
    def _set_lhs_tables(
        ...
        return_none = False
        if (
            (self.n_factors_l is None) and (self.n_factors_x is None)
        ) or self.n_trials is None:
            return_none = True

        self.arr_lhs_l, self.arr_lhs_x = (
            (None, None) if return_none else self.generate_lhs()
        )
    ...
    def generate_lhs(
        ...
        if retun_none:
            self._log(
                "Warning in generate_lhs: one or more elements are missing. If not initialized with n_factors_l, n_factors_x, or n_trials, update with self._set_lhs",
                type_log="warning",
            )
            return None
        ...
        df_lhs_l = (
            pyd.lhs(n_factors_l, n_trials, random_state=rs_l)
            if (n_factors_l is not None)
            else None
        )
        df_lhs_x = (
            pyd.lhs(n_factors_x, n_trials, random_state=rs_x)
    ...
    def retrieve_lhs_tables_by_design(
        self,
        ...
        ignore_trial_flag = (
            self.ignore_trial_flag if (ignore_trial_flag is None) else ignore_trial_flag
        )
        return_type = (
            self.default_return_type
            if (
                (return_type is None) or (return_type not in [pd.DataFrame, np.ndarray])
            )
            else return_type
        )
        ...
        return_type = (
            pd.DataFrame
            if (return_type not in [pd.DataFrame, np.ndarray])
            else return_type
        )
        ...
        if design_id is None:
            ...
            attr_design_id = (
                self.attribute_design_id if (attr_design_id is None) else attr_design_id
            )
            if design_id not in attr_design_id.key_values:
                design_base_assumed = min(attr_design_id.key_values)
                self._log(
                    f"Error in retrieve_lhs_tables_by_design: invalid design_id '{design_id}'. Defaulting to design_id '{design_base_assumed}'.",
                    type_log="warning",
                )
            ...
            if arr_lhs_l is not None:
                arr_lhs_out_l = (
                    np_trans_strat(arr_lhs_l, design_id, attr_design_id)
                    if vary_l_q
                    else np.ones(arr_lhs_l.shape)
                )
            ...
            if arr_lhs_x is not None:
                arr_lhs_out_x = (
                    arr_lhs_x
                    if vary_x_q
                    else ignore_trial_flag * np.ones(arr_lhs_x.shape)
                )

        if return_type == pd.DataFrame:
            dict_keys = {field_lhs_key: self.vector_lhs_key_values}
            dict_keys.update({attr_design_id.key: design_id}) if (
            ...
            arr_lhs_out_x = (
                ...
                if (arr_lhs_out_x is not None)
                else arr_lhs_out_x
            )

        return arr_lhs_out_l, arr_lhs_out_x

    def transform_strategy_lhs_trial_from_design(
        self,
        x: Union[float, int],
        design_id: int,
        ...
        field_transform_b = (
            self.field_transform_b if (field_transform_b is None) else field_transform_b
        )
        field_transform_m = (
            self.field_transform_m if (field_transform_m is None) else field_transform_m
        )
        field_transform_inf = (
            self.field_transform_inf
            if (field_transform_inf is None)
            else field_transform_inf
        )
        field_transform_sup = (
            self.field_transform_sup
            if (field_transform_sup is None)
            else field_transform_sup
        )
