...
class OrderedDirectProductTable:
    ...
    def _initialize_cumulative_dim_products(
        self,
    ) -> None:
    ...
    def _initialize_dims(
        ...
        dict_dim_cardinalities = dict(
            (k, len(v)) for k, v in dict_values_by_dim.items()
        )
        dims_ordered_reversed = list(reversed(dims_ordered))
        cardinality_ordered = [dict_dim_cardinalities.get(x) for x in dims_ordered]
        cardinality_ordered_reversed = [
            dict_dim_cardinalities.get(x) for x in dims_ordered_reversed
        ]

        dict_values_to_index_by_dim = dict(
            (k, dict(zip(v, range(len(v)), strict=False))) for k, v in dict_dims.items()
        )
        dict_index_to_values_by_dim = dict(
            (k, dict(zip(range(len(v)), v, strict=False))) for k, v in dict_dims.items()
        )
    ...
    def _initialize_moving_cardinality(
        self,
    ...
    def get_dims_from_key(
        ...
        out = (
            ...
            else dict(zip(self.dims_ordered, out, strict=False))
        )

        return out

    def get_indexing_dataframe(
        self,
        ...
        key_values = (
            ...
            if (key_values is None)
                ...
                if not isinstance(key_values, dict)
                    ...
                    (k, v) for k, v in key_values.items() if k in self.dims_ordered
                )
            )
        )
        ...
        use_intersection = (
            True if (key_dict_logic not in ["and", "or"]) else (key_dict_logic == "and")
        )
        ...
        if keep_index is None:
            for key in enumerate(self.dims_ordered):
                ...
                if key in key_values.keys():
                    ...
                    inds = [
                        self.values_to_indices_by_dim.get(key).get(x) for x in vals_keep
                    ]
                    ...
                    keep_index = (
                        ...
                        if (keep_index is None)
                            ...
                            keep_index.intersection(set(w))
                            if use_intersection
                            else keep_index.union(set(w))
                        )
                    )
    ...
    def get_indexing_dataframe_from_primary_key(
        self,
