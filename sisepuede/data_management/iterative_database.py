...
class IterativeDatabaseTable:
    ...
    def check_table_indices(
        ...
        if (self.fields_index is None) and (
            (not self.exists) or (self.columns is None)
        ):
            return None
        ...
        if self.fields_index is not None:
            if not set(self.fields_index).issubset(df_addition.columns):
                missing_cols = sf.print_setdiff(
                    set(self.fields_index),
                    set(df_addition.columns),
                )
        ...
        new_indices = set(
            [
                tuple(x)
                for x in np.array(df_addition[self.fields_index].drop_duplicates())
            ],
        )
    ...
    def _clean_stash(
        ...
        if self.keep_stash:
            self.stash = {}
            self._log("Cleaned stash", type_log="info")

    def filter_data_frame_from_index_tuples(
        self,
    ...
    def _initialize_conflict_resolution(
        self,
        ...
        self.index_conflict_resolution = (
            default_resolution
            if (index_conflict_resolution not in self.valid_resolutions)
            else index_conflict_resolution
        )
    ...
    def _initialize_engine(
        ...
        engine = (
            engine
            if (isinstance(engine, str) or isinstance(engine, sqlalchemy.engine.Engine))
            else os.getcwd()
        )
        ...
        if self.interaction_type == "sql":
            ...
            if self.exists:
                ...
                if not replace_on_init:
                    ...
                    try:
                        self._log(
                            f"Trying to get columns for {self.table_name} with query: {query}",
                            type_log="debug",
                        )
                        ...
                        self._log(
                            f"Table {self.table_name} found in sql connection; it will not be replaced.",
                            type_log="info",
                        )
                    ...
                    if df_columns is not None:
                        ...
                        self._log("\tColumns successfully retrieved.", type_log="info")
            ...
            if self.exists:
                ...
                if not replace_on_init:
                    ...
                    try:
                        self._log(
                            f"Trying to get columns for {self.table_name} from table at {fp_table}.",
                            type_log="debug",
                        )
                        df_columns = pd.read_csv(fp_table, nrows=0)

                        self._log(
                            f"Table {self.table_name} found at {fp_table}; it will not be replaced.",
                            type_log="info",
                        )

                    except Exception as e:
                        self._log(
                            f"Attempt to access CSV at {fp_table} failed with error: {e}",
                            type_log="error",
                        )
                    ...
                    self._log(
                        f"Table {self.table_name} was found at {fp_table} and was successsfully removed. Columns will be initialized on the first write.",
                        type_log="info",
                    )
                    self.exists = False

    def _initialize_table_indices(
        self,
        fields_index: Union[List[str], None] = None,
        ...
        if (fields_index is None) or (fields_index == []):
            self._log(
                "No index fields defined. Index field values will not be checked when writing to tables.",
                type_log="warning",
            )
            return
        ...
        if (not self.exists) or (self.columns is None):
            self._log(
                f"No index fields found in {self.table_name}. Initializing index fields.",
                type_log="warning",
            )
        ...
        self.available_indices = (
            set() if (len(self.fields_index) > 0) else self.available_indices
        )
        ...
        if self.interaction_type == "sql":
            try:
                ...
                self._log(
                    f"Error trying to initialize available indices from SQL: {e}",
                    type_log="error",
                )
                return
            ...
            try:
                ...
                self._log(
                    f"Error trying to initialize available indices from CSV at {self.fp_table}: {e}",
                    type_log="error",
                )
                return
        ...
        self._log(
            f"Successfully initiatlized available indices from {str_log_msg}",
            type_log="info",
        )
    ...
    def _stash(
        ...
        if (
            isinstance(df_stash, pd.DataFrame)
            and self.keep_stash
            and isinstance(self.stash, dict)
        ):
            self.stash.update({key: df_stash})
            shp = str(df_stash.shape)
            self._log(
                f"Successfully stashed data frame with shape {shp}",
                type_log="info",
            )
        else:
    ...
    def _verify_existence(
        ...
        * self.exists
        ...
        if self.interaction_type == "sql":
            ...
            if check_columns and self.exists:
                try:
                    ...
                    self._log(
                        f"CSV read from {self.fp_table} failed with error: {e}",
                        type_log="error",
                    )
                    cols_cur = []
        ...
        if check_columns:
            ...
            if (self.columns is None) and self.exists:
    ...
    def _check_columns_and_index_on_write(
        self,
        df_write: pd.DataFrame,
    ) -> None:
        ...
        * Sets the following properties if unset:
        ...
        set_ind_check = (
            set(self.fields_index) if (self.fields_index is not None) else set({})
        )
    ...
    def read_table(
        ...
        **kwargs,
        ...
        if self.interaction_type == "sql":
            query_append = (
                sqlutil.dict_subset_to_query_append(
                    dict_subset,
                    query_logic=query_logic,
                )
                if (dict_subset is not None)
                else None
            )
            ...
            try:
                ...
                self._log(
                    f"Error trying to read table {self.table_name} via {self.interaction_type}: {e}",
                    type_log="error",
                )
                return None
    ...
    def resolve_index_conlicts(
        ...
        self._log(
            f"Running conflict resolution '{index_conflict_resolution}' for table {self.table_name}.",
            type_log="warning",
        )
        ...
        if index_conflict_resolution == "skip":
        ...
        if (index_conflict_resolution == "write_replace") and (
            self.interaction_type == "sql"
        ):
        ...
        skip_and_stash |= (self.interaction_type == "csv") and (
            index_conflict_resolution == "write_replace"
        )
    ...
    def _write_to_table(
        ...
        if self.interaction_type == "sql":
            try:
                ...
                self.exists = True
                self.available_indices.update(set_indices_to_write) if (
                    set_indices_to_write is not None
                ) else None
                self._log(
                    f"Table {self.table_name} successfully {write_str} to database.",
                    type_log="info",
                )

            except Exception as e:
                self._log(
                    f"Error in _write_to_table trying to write {self.table_name}: {e}",
                    type_log="error",
                )
            ...
            try:
                ...
                self._log(
                    f"Table {self.table_name} successfully {write_str} to {self.fp_table}.",
                    type_log="info",
                )
...
class IterativeDatabase:
    ...
    def get_idt_engine(
        ...
        engine = (
            self.engine
            if (self.interaction_type in ["sql", "sqlite"])
            else self.fp_base_output
        )
        return engine

    def _initialize_analysis_id(
        self,
    ...
    def _initialize_engine(
        ...
        default_engine_str = (
            "sqlite"
            if (default_engine_str not in self.valid_engine_strs)
            else default_engine_str
        )
        ...
        if not (
            ...
            self._log(
                f"Invalid export engine type {tp} specified: setting to str '{default_engine_str}'.",
                type_log="warning",
            )
        ...
        if isinstance(engine, str):
            ...
            self.interaction_type = (
                default_engine_str if (engine not in self.valid_engine_strs) else engine
            )
            self._log(
                f"\tSetting export engine to '{self.interaction_type}'.",
                type_log="info",
            )
            ...
            if isinstance(fp_base_output, str):
                ...
                dir_output = (
                    fp_base_output
                    if (self.interaction_type == "csv")
                    else os.path.dirname(fp_base_output)
                )
            ...
            self.engine = engine
            self.interaction_type = "sql"
            self._log("\tSetting export engine to SQLAlchemy engine.", type_log="info")
    ...
    def _initialize_tables(
        ...
        self.dict_all_tables = (
            {} if not isinstance(dict_all_tables, dict) else dict_all_tables
        )
        self.all_tables = sorted(list(self.dict_all_tables.keys()))

        # set engine and some defaults
        engine = self.get_idt_engine()
        self.fields_index_default = (
            fields_index_default if isinstance(fields_index_default, list) else None
        )
        ...
        for table_name in enumerate(self.all_tables):
            ...
            try:
                ...
                self.index_conflict_resolution = (
                    ...
                    if (self.index_conflict_resolution is None)
                    else self.index_conflict_resolution
                )
    ...
    def _write_to_table(
        ...
        index_conflict_resolution = (
            None
            if (index_conflict_resolution not in table.valid_resolutions)
            else index_conflict_resolution
        )
    ...
    def _add_tables(
        ...
        dict_new_tables = (
            {} if not isinstance(dict_new_tables, dict) else dict_new_tables
        )
        dict_tables_merge = {}
        ...
        self.dict_all_tables.update(
            dict(
                (k, v) for k, v in dict_new_tables.items() if (k in dict_tables_merge)
            ),
        )
        ...
        for table_name in table_names:
            ...
            if table_name in self.all_tables:
                try:
                    ...
                    self._log(
                        f"Error trying to drop table {table_name}: {e}",
                        type_log="error",
                    )
