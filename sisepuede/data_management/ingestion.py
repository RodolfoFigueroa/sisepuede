...
class InputTemplate:
    ...
    def template_from_inputs(
        ...
        regex_max = (
            ...
            if not isinstance(regex_max, re.Pattern)
            else regex_max
        )
        regex_min = (
            self.regex_template_min
            if not isinstance(regex_min, re.Pattern)
            else regex_min
        )
        regex_tp = (
            self.regex_template_time_period
            if not isinstance(regex_tp, re.Pattern)
            else regex_tp
        )
        ...
        trajgroup_passed_to_base_q = (
            field_req_variable_trajectory_group in df_base.columns
        )

        # melt input dataframe to long and filter
        fields_id = [
            x for x in df_input.columns if x in [field_key_strategy, field_time_period]
        ]
        ...
        for strat, df in df_input_merge:
            ...
            df_merge = (
                ...
                & (field_req_variable_trajectory_group in df.columns)
                else df
            )
        ...
        fields_skip_merge = (
            [field_req_variable_trajectory_group] if trajgroup_passed_to_base_q else []
        )
        ...
        if isinstance(df_variable_information, pd.DataFrame):
            ...
            field_max_nms = [
                x
                for x in df_variable_information.columns
            ...
            dict_rnm = {}
            dict_rnm.update({field_max_nms: field_max}) if (
                field_max_nms in df_var_info.columns
            ) else None
            dict_rnm.update({field_min_nms: field_min}) if (
                field_min_nms in df_var_info.columns
            ) else None
    ...
    def _initialize_attribute_strategy(
        self,
    ...
    def _initialize_basic_required_fields(
        self,
    ...
    def _initialize_model_attributes(
        self,
        model_attributes: ma.ModelAttributes,
    ) -> None:
    ...
    def _initialize_regex_patterns(
        self,
        regex_max: re.Pattern = re.compile(r"max_(\d*$)"),
        regex_min: re.Pattern = re.compile(r"min_(\d*$)"),
        regex_tp: re.Pattern = re.compile(r"(\d*$)"),
        ...
        * self.regex_template_time_period
        ...
        self.regex_template_max = (
            regex_max
            if isinstance(regex_max, re.Pattern)
            else re.compile(r"max_(\d*$)")
        )
        self.regex_template_min = (
            regex_min
            if isinstance(regex_min, re.Pattern)
            else re.compile(r"min_(\d*$)")
        )
        self.regex_template_time_period = (
            regex_tp if isinstance(regex_tp, re.Pattern) else re.compile(r"(\d*$)")
        )
    ...
    def name_field_maxmin_from_index(
        self,
        index: int,
        ...
        if regex in ["max", "min"]:
            regex = (
                self.regex_template_max if (regex == "max") else self.regex_template_min
            )
    ...
    def read_template(
        ...
        - dict_outputs: dictionary {int: pd.DataFrame} mapping a strategy id to
                the associated dataframe
        - dict_sheet_to_strategy: dictionary mapping a sheet to a strategy name
        - field_max: field specifying the maximum scalar in the final time
                period
        - field_min: field specifying the minimum scalar in the final time
                period
        - fields_tp: fields specifying the time periods

        Function Arguments
        ------------------
        - template_input: file path (str) to Excel Template or input dictionary
                (dict) with keys matching InputTemplate.regex_sheet_name
        ...
        for k in sheets_iterate:
            ...
            dict_field_tp_to_tp, df_template_sheet, field_min, field_max, fields_tp = (
                tup
            )

            # check time period fields
            all_time_period_fields = (
                set(fields_tp)
                if (all_time_period_fields is None)
                else (all_time_period_fields & set(fields_tp))
            )
            ...
            df_template_sheet = df_template_sheet[
                self.list_fields_required_base + [field_min, field_max] + fields_tp
            ]
    ...
    def build_inputs_by_strategy(
        ...
        Keyword Arguments:
        -----------------
        - dict_strategies_to_sheet: dictionary of type {int -> pd.DataFrame}
                that maps a strategy id to its associated template sheet
                * If None (default), uses InputTemplate.dict_strategy_id_to_sheet
        - strategies_include: list or list-like (np.array) of strategy ids to
        ...
        strategies_include = [strat_base] + [
            x for x in strategies_include if (x != strat_base)
        ]
        ...
        for strat in strategies_include:
            ...
            if strat != strat_base:
                ...
                df_sheet_base = df_out[0][
                    ~df_out[0][self.field_req_variable].isin(vars_cur)
                ].copy()
                df_sheet_base[self.model_attributes.dim_strategy_id] = df_sheet_base[
                    self.model_attributes.dim_strategy_id
    ...
    def verify_and_return_sheet_time_periods(
        self,
        df_in: pd.DataFrame,
        regex_max: Union[re.Pattern, None] = None,
        regex_min: Union[re.Pattern, None] = None,
        ...
        fields_tp = [
            regex_tp.match(str(x))
            for x in df_in.columns
            if (regex_tp.match(str(x)) is not None)
        ]
        ...
        fields_valid = [
            x.string
            for x in fields_tp
            if (dict_field_tp_to_tp.get(x.string) in attr_tp.key_values)
        ]
    ...
    def verify_input_template_sheet(
        self,
        ...
        *NOTE*: returns None if errors occur trying to load, so return values
        ...
        Function Arguments
        ------------------
        - df_template_sheet: A data frame representing the input template sheet
                by strategy

        Keyword Arguments:
        -----------------
        - base_strategy_q: running the base strategy? If so, requirements for
                input variables are different.
        - sheet_name: name of the sheet passed for error handling and
                troubleshooting
...
class BaseInputDatabase:
    ...
    def get_regions(
        ...
        if regions is None:
            ...
            regions_out = [
                self.model_attributes.clean_region(region) for region in regions
            ]
            regions_out = [
                region for region in regions_out if region in attr_region.key_values
            ]
                ...
                if df_template_db is not None:
                    ...
                    if all_fields is not None:
                        if not set(df_template_db.columns).issubset(all_fields):
                            ...
                            df_template_db.drop(fields_drop, axis=1, inplace=True) if (
                                len(fields_drop) > 0
                            ) else None
                    else:
    ...
    def get_template_path(
        ...
        **kwargs,
        ...
        fp_templates = (
            self.fp_templates if not isinstance(fp_templates, str) else fp_templates
        )
        ...
        if not demo_q:
            # check
            if region is None:
                msg = "Invalid specification of region: a region must be specified unless the database is initialized in demo mode."
