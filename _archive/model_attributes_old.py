...
class Configuration:
    ...
    def check_config_defaults(
        ...
        val_list = (
            [vals] if ((not isinstance(vals, list)) or set_as_literal_q) else vals
        )
    ...
    def get_config_information(
        ...
        attr_monetary = (
            attr_monetary if (attr_monetary is not None) else self.attr_monetary
        )
        ...
        attr_time_period = (
            attr_time_period
            if (attr_time_period is not None)
            else self.attr_time_period
        )
        ...
        if attr_parameters_required is not None:
            ...
            if (
                ...
                and (dict_key_to_required_param is not None)
                and (dict_key_to_default_value is not None)
            ):
                # add defaults
                for k in attr_parameters_required.key_values:
                    param_config = (
                        dict_key_to_required_param.get(k)
    ...
    def get_valid_values_from_attribute_column(
        self,
        ...
        if field_map_to_val != None:
            if field_map_to_val in attribute_table.field_maps.keys():
                ...
                raise KeyError(
                    f"Error in get_valid_values_from_attribute_column: the field map '{field_map_to_val}' is not defined.",
                )
...
class ModelAttributes:
    ...
    def _initialize_all_primary_category_flags(
        self,
    ) -> list:
        ...
        * all_primary_category_flags
        ...
        if attr_subsec is not None:
            ...
            all_pcflags = [
                x.replace("`", "")
                for x in all_pcflags
                if sf.clean_field_names([x])[0] in self.all_pycategories
            ]

        self.all_primary_category_flags = all_pcflags

    def _initialize_attribute_tables(
        self,
        dir_att: str,
        ...
        all_types = [
            x
            for x in os.listdir(dir_att)
            if (self.attribute_file_extension in x)
            and (
                (self.substr_categories in x)
                or (self.substr_varreqs_allcats in x)
                or (self.substr_varreqs_partialcats in x)
        ...
        for att in all_types:
            ...
            if self.substr_dimensions in att:
                ...
                if nm is None:
                    nm = sf.clean_field_names(
                        [att.replace("attribute_", "").replace(".csv", "")],
                    )[0]
            ...
            elif (self.substr_varreqs_allcats in att) or (
                ...
                raise ValueError(
                    f"Invalid attribute '{att}': ensure '{self.substr_categories}', '{self.substr_varreqs_allcats}', or '{self.substr_varreqs_partialcats}' is contained in the attribute file.",
                )
        ...
        if not set({table_name_attr_sector, table_name_attr_subsector}).issubset(
    ...
    def _initialize_basic_dimensions_of_analysis(
        self,
    ) -> None:
    ...
    def _initialize_basic_other_properties(
        self,
    ) -> None:
    ...
    def _initialize_basic_table_names_nemomod(
        self,
    ) -> None:
    ...
    def _initialize_basic_subsector_names(
        self,
    ) -> None:
    ...
    def _initialize_basic_template_substrings(
        self,
    ) -> None:
    ...
    def _initialize_basic_varchar_components(
        self,
    ) -> None:
    ...
    def _initialize_emission_modvars_by_gas(
        self,
        ...
        for tab in all_tabs:
            ...
            modvars = list(
                tab[tab[self.field_emissions_total_flag] == 1]["variable"],
            )
    ...
    def _initialize_gas_attributes(
        self,
    ) -> None:
        ...
        * self.dict_gas_to_fc_designation
        ...
        dict_gas_to_fc_designation = dict(
            sum(
                [[(x, k) for x in v] for k, v in dict_fc_designation_to_gas.items()],
                [],
            ),
        )
    ...
    def _initialize_other_dictionaries(
        self,
    ) -> None:
    ...
    def _initialize_sector_sets(
        ...
        * self.emission_subsectors
        ...
        l_with = sorted(
            list(attr_subsec.field_maps["subsector_to_primary_category_py"].keys()),
        )
    ...
    def _initialize_variables_by_subsector(
        self,
    ) -> None:
        ...
        for subsector in self.all_subsectors_with_primary_category:
            ...
            for modvar in vars_by_subsector:
                ...
                dict_fields_to_vars.update(
                    dict(zip(var_lists, [modvar for x in var_lists], strict=False)),
                )

                all_variables_input += (
                    var_lists if modvar in vars_by_subsector_input else []
                )
                all_variables_output += (
                    var_lists if modvar in vars_by_subsector_output else []
                )
    ...
    def _check_binary_category_partition(
        self,
        attr: AttributeTable,
        fields: List[str],
    ...
    def _check_binary_fields(
        ...
        for fld in fields:
            ...
            if not all([((x == 1) | (x == 0)) for x in list(attr.table[fld])]):
                raise ValueError(
    ...
    def _check_numeric_fields(
        ...
        for fld in fields:
            ...
            try:
                ...
                raise ValueError(
                    f"Error in subsector {subsec}: Non-numeric values found in field '{fld}'. Check the table at '{attr.fp_table}'.",
                )
            ...
            if check_bounds is not None:
                if (min(vals) < check_bounds[0]) or (max(vals) > check_bounds[1]):
                    raise ValueError(
                        f"Error in subsector {subsec}: values in field '{fld}' outside of bounds ({check_bounds[0]}, {check_bounds[1]}) specified. Check the attribute table at '{attr.fp_table}'.",
                    )
            elif nonnegative_q and (min(vals) < 0):
                raise ValueError(
                    f"Error in subsector {subsec}: Negative values found in field '{fld}'. The field should only have non-negative numbers. Check the table at '{attr.fp_table}'.",
                )
            ...
            if integer_q:
                ...
                if not all(vals_check):
                    raise ValueError(
                        f"Error in subsector {subsec}: Non-integer equivalent values found in the field {fld}. Entries in '{fld}' should be integers or float equivalents. Check the table at '{attr.fp_table}'.",
                    )

    def _check_subsector_attribute_table_crosswalk(
        self,
        ...
        if type_primary not in valid_types:
            raise ValueError(
                f"Invalid type_primary '{type_primary}' specified. Valid values are '{str_valid_types}'.",
            )
        if type_target not in valid_types:
            raise ValueError(
                f"Invalid type_target '{type_target}' specified. Valid values are '{str_valid_types}'.",
            )
        ...
        if type(dict_subsector_primary) == dict:
            if len(dict_subsector_primary) != 1:
                raise KeyError(
                    "Error in dictionary dict_subsector_primary: only one key (subsector_primary) should be specified.",
                )
            ...
            raise ValueError(
                f"Invalid type '{t_str}' of dict_subsector_primary: 'dict' and 'str' are acceptable values.",
            )
        ...
        if not key_primary:
            raise ValueError(
                f"Invalid type_primary '{type_primary}' specified for primary subsector '{subsector_primary}': type not found.",
            )
        ...
        if not key_primary:
            raise ValueError(
                f"Invalid type_primary '{type_target}' specified for primary subsector '{subsector_target}': type not found.",
            )
        ...
        if field_subsector_primary not in attr_prim.table.columns:
            raise ValueError(
                f"Error in _check_subsector_attribute_table_crosswalk: field '{field_subsector_primary}' not found in the '{subsector_primary}' attribute table. Check the file at '{attr_prim.fp_table}'.",
            )
        ...
        if allow_multiple_cats_q:
            primary_cats_defined = (
                sum(
                    [
                        [clean_schema(y) for y in x.split(self.delim_multicats)]
        ...
        if not set(primary_cats_defined).issubset(set(attr_targ.key_values)):
            ...
            raise ValueError(
                f"Invalid categories {invalid_vals} specified in field '{field_subsector_primary}' of the {subsector_primary} attribute table at '{attr_prim.fp_table}'.\n\nValid categories from {subsector_target} are: {valid_vals}",
            )
        ...
        if injection_q:
            ...
            if len(set(primary_cats_defined)) != len(primary_cats_defined):
                ...
                raise ValueError(
                    f"Error in {subsector_primary} attribute table at '{attr_prim.fp_table}': duplicate specifications of target categories {duplicate_vals}. There map of {subsector_primary} categories to {subsector_target} categories should be an injection map.",
                )
    ...
    def _check_attribute_tables_abv_subsector(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_agrc(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_enfu(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_enst(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_entc(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_inen(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_ippu(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_lndu(
        self,
    ) -> None:
        ...
        if not set_cats_forest_in_land_use.issubset(set(cats_landuse)):
            ...
            raise KeyError(
                f"Missing key values in land use attribute file '{attribute_landuse.fp_table}': did not find land use categories {missing_str}.",
            )
        if not set_land_use_forest_cats.issubset(cats_forest):
            extra_vals = set_land_use_forest_cats - set(cats_forest)
            extra_vals = sf.format_print_list(extra_vals)
            raise KeyError(
                f"Undefined forest categories specified in land use attribute file '{attribute_landuse.fp_table}': did not find forest categories {extra_vals}.",
            )
        ...
        if len(set(cats_crop) & set(cats_mnrl)) > 0:
            raise ValueError(
                f"Invalid specification of field '{field_mnrl}' in {self.subsec_name_lndu} attribute located at {attribute_landuse.fp_table}. Category '{cats_crop[0]}' cannot be specified as a target category.",
            )
    ...
    def _check_attribute_tables_lsmm(
        self,
    ) -> None:
        ...
        for field in fields_check_sum:
            ...
            if (not vals.issubset(set({0, 1}))) or (sum(attr.table[field]) > 1):
                raise ValueError(
                    f"Invalid specification of field '{field}' in {subsec} attribute located at {attr.fp_table}. Check to ensure that at most 1 is specified; all other entries should be 0.",
                )
        ...
        if max(vec_max) > 1:
            ...
            raise ValueError(
                f"Invalid specification of fields {fields} in {subsec} attribute located at {attr.fp_table}: Non-injective mapping specified--categories can map to at most 1 of these fields.",
            )

    def _check_attribute_tables_trde(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_trns(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_wali(
        self,
    ) -> None:
    ...
    def _check_attribute_tables_waso(
        self,
    ) -> None:
    ...
    def add_index_fields(
        ...
        # update values
        dict_indices.update({self.dim_design_id: design_id}) if (
            design_id is not None
        ) else None
        dict_indices.update({self.dim_future_id: future_id}) if (
            future_id is not None
        ) else None
        dict_indices.update({self.dim_primary_id: primary_id}) if (
            primary_id is not None
        ) else None
        dict_indices.update({self.dim_region: region}) if (region is not None) else None
        dict_indices.update({self.dim_strategy_id: strategy_id}) if (
            strategy_id is not None
        ) else None
        dict_indices.update({self.dim_time_period: time_period}) if (
            time_period is not None
        ) else None
        dict_indices.update({self.dim_time_series_id: time_series_id}) if (
            time_series_id is not None
        ) else None
    ...
    def check_dimensions_of_analysis(
        self,
    ) -> None:
        """Ensure dimensions of analysis are properly specified"""
        if not set(self.sort_ordered_dimensions_of_analysis).issubset(
            set(self.all_dims),
        ):
            missing_vals = sf.print_setdiff(
    ...
    def check_modvar(
        ...
        out = modvar if modvar in self.all_model_variables else None
    ...
    def check_restricted_value_argument(
        self,
        arg: Any,
        valid_values: list,
        func_arg: str = "",
    ...
    def clean_dimension_fields(
        ...
        """
        fields_clean = [
            x for x in self.sort_ordered_dimensions_of_analysis if x in df_in.columns
        ]
    ...
    def get_all_subsector_emission_total_fields(
        self,
        ...
        subsectors_emission = (
            list(
                attr.table[attr.table["emission_subsector"] == 1]["subsector"],
            )
    ...
    def get_attribute_table(
        ...
        if table_type in ["key_varreqs_all", "key_varreqs_partial"]:
            key_dict = self.get_subsector_attribute(subsector, table_type)
            return self.dict_varreqs.get(key_dict)
        raise ValueError(
            f"Invalid table_type '{table_type}': valid options are 'pycategory_primary', 'key_varreqs_all', 'key_varreqs_partial'.",
        )

    def get_baseline_scenario_id(
        self,
        dim: str,
        ...
        if field_check not in attr.table:
            ...
            if len(tab_red) > 1:
                ret = tab_red[0]
                warnings.warn(
                    f"Multiple baselines specified for dimension {dim}. Ensure that only baseline is set in the attribute table at '{tab.fp_table}'. Defaulting to minimum value of {ret}.",
                )
    ...
    def get_categories_from_attribute_characteristic(
        self,
        subsector: str,
        dict_subset: dict,
        ...
        """
        auto_select_attr_q = (
            self.check_subsector(subsector, throw_error_q=False) == subsector
        )
    ...
    def get_category_replacement_field_dict(
        self,
        modvar: str,
    ) -> Union[dict, None]:
    ...
    def get_df_dimensions_of_analysis(
        self,
        df_in: pd.DataFrame,
        ...
        if type(df_in_shared) == pd.DataFrame:
            cols = [
                x
                for x in self.sort_ordered_dimensions_of_analysis
                if (x in df_in.columns) and (x in df_in_shared.columns)
            ]
        else:
            cols = [
                x
    ...
    def get_dimensional_attribute(
        ...
        if out_val is None:
            ...
            warnings.warn(
                f"Invalid dimensional attribute '{return_type}'. Valid return type values are:{valid_rts}",
            )

        return out_val

    def get_emission_subsectors(
        self,
    ) -> List[str]:
    ...
    def get_fluorinated_compound_dictionaries(
        self,
    ...
    def get_ordered_category_attribute(
        self,
        subsector: str,
        attribute: str,
        ...
        if attr_type == "pycategory_primary":
            ...
            raise ValueError(
                f"Invalid attribute type '{attr_type}': select 'pycategory_primary', 'key_varreqs_all', or 'key_varreqs_partial'.",
            )

        if attribute not in attr_cur.table.columns:
            raise ValueError(
                f"Missing attribute column '{attribute}': attribute not found in '{subsector}' attribute table.",
            )

        # get the dictionary and order
        tab = (
            attr_cur.table[attr_cur.table[attribute] != "none"]
            if skip_none_q
            else attr_cur.table
        )
        dict_map = (
            sf.build_dict(tab[[attr_cur.key, attribute]])
            if (not clean_attribute_schema_q)
            else dict(
                zip(
                    tab[attr_cur.key],
                    list(tab[attribute].apply(clean_schema)),
                    strict=False,
                ),
            )
        )
    ...
    def get_ordered_vars_by_nonprimary_category(
        self,
        subsector_var: str,
        subsector_targ: str,
        varreq_type: str,
    ...
    def get_primary_category_schema_element(
        self,
        subsector: str,
    ) -> Union[str, None]:
    ...
    def get_region_list_filtered(
        ...
        """
        attribute_region = (
            self.dict_attributes.get("region")
        ...
        regions = (
            [x for x in regions if x in attribute_region.key_values]
    ...
    def get_sector_subsectors(
        ...
        field_ext = "subsector" if return_type in ["name"] else attr.key
    ...
    def get_subsector_attribute(
        ...
        dict_out = {
            "pycategory_primary": self.dict_attributes[
                self.table_name_attr_subsector
            ].field_maps["subsector_to_primary_category_py"][subsector],
            "abv_subsector": self.dict_attributes[
                self.table_name_attr_subsector
            ].field_maps["subsector_to_abbreviation_subsector"][subsector],
        }

        dict_out.update(
            {
                "sector": self.dict_attributes[
                    self.table_name_attr_subsector
                ].field_maps["abbreviation_subsector_to_sector"][
                    dict_out["abv_subsector"]
                ],
            },
        )
        dict_out.update(
            {
                "abv_sector": self.dict_attributes[
                    self.table_name_attr_sector
                ].field_maps["sector_to_abbreviation_sector"][dict_out["sector"]],
            },
        )

        # format some strings
        key_allvarreqs = (
            self.substr_varreqs_allcats.replace(self.substr_varreqs, "")
            + dict_out["abv_sector"]
            + "_"
            + dict_out["abv_subsector"]
        )
        key_partialvarreqs = (
            self.substr_varreqs_partialcats.replace(self.substr_varreqs, "")
            + dict_out["abv_sector"]
            + "_"
            + dict_out["abv_subsector"]
        )
        ...
        if return_type in dict_out:
            return dict_out[return_type]

        # warn user, but still allow a return
        valid_rts = sf.format_print_list(list(dict_out.keys()))
        warnings.warn(
            f"Invalid subsector attribute '{return_type}'. Valid return type values are:{valid_rts}",
        )
    ...
    def get_valid_categories(
        ...
        categories = (
            ...
            else (attr.key_values if (categories is None) else [])
        )
        categories = None if (len(categories) == 0) else categories

        return categories

    def get_var_dicts_by_shared_category(
        self,
        subsector: str,
        category_pivot: str,
        ...
        for table_type in ["key_varreqs_all", "key_varreqs_partial"]:
            ...
            for field in cols:
                df_tmp = attr_table.table[attr_table.table[field] == 1][
                    [category_pivot, "variable"]
                ].copy()
                df_tmp[category_pivot] = df_tmp[category_pivot].apply(clean_schema)
                dict_out.update(
                    {field: sf.build_dict(df_tmp[[category_pivot, "variable"]])},
                )
    ...
    def merge_array_var_partial_cat_to_array_all_cats(
        self,
        ...
        if (modvar is None) and any(
            ...
            raise ValueError(
                "Error in input specification. If modvar == None, then output_cats and output_subsec cannot be None.",
            )

        if not sf.isnumber(missing_vals):
            raise ValueError(
                "Error in input specification of missing_vals: missing_vals should be a floating point number of integer.",
            )
        ...
        if modvar is not None:
            ...
            cat_restriction_type = self.dict_model_variable_to_category_restriction[
                modvar
            ]
        else:
            ...
            if not set(output_cats).issubset(set(attr_subsec.key_values)):
                ...
                raise ValueError(
                    f"Error in merge_array_var_partial_cat_to_array_all_cats: Invalid categories {invalid_values} specified for subsector {subsector} in output_cats.",
                )
            # check that all categories are unique
            if len(set(output_cats)) != len(output_cats):
                raise ValueError(
                    "Error in merge_array_var_partial_cat_to_array_all_cats: Categories specified in output_cats are not unique. Check that categories are unique.",
                )
        ...
        if cat_restriction_type == "all":
            return array_vals
        array_default = (
            np.ones((len(array_vals), attr_subsec.n_key_values)) * missing_vals
        )
        cats = (
            self.get_variable_categories(modvar)
            if (modvar is not None)
            else output_cats
        )
    ...
    def reduce_all_cats_array_to_partial_cat_array(
        self,
        array_vals: np.ndarray,
        modvar: str,
    ) -> np.ndarray:
    ...
    def get_unit_equivalent(
        ...
        if unit not in attr_cur.key_values:
            ...
            if unit_type_str in attr_cur.table.columns:
                ...
                unit = (
                    dict_convert_unit.get(unit)
                    if (dict_convert_unit is not None)
                    else None
                )
        ...
        if key_dict not in attr_cur.field_maps.keys():
            valid_units_to_match = sf.format_print_list(valid_units).lower()
            raise KeyError(
                f"Invalid {unit_type_str} to match '{unit_to_match}': defined {unit_type_str} units to match are {valid_units_to_match}.",
            )
        ...
        if out is None:
            ...
            raise KeyError(
                f"Invalid {unit_type_str} '{unit}': defined {unit_type_str} units are {valid_vals}.",
            )

        return out

    def get_area_equivalent(
        self,
        area: str,
    ...
    def get_energy_power_swap(
        ...
        secondary_key_values_energy = attr_ener.field_maps.get(
            f"{attr_ener.key}_to_{name_energy_str_clean}",
        )
        secondary_key_values_energy = (
            list(secondary_key_values_energy.values())
            if (secondary_key_values_energy is not None)
            else secondary_key_values_energy
        )
        ...
        secondary_key_values_power = attr_powr.field_maps.get(
            f"{attr_powr.key}_to_{name_power_str_clean}",
        )
        secondary_key_values_power = (
            list(secondary_key_values_power.values())
            if (secondary_key_values_power is not None)
            else secondary_key_values_power
        )
        ...
        if input_unit in (set(attr_ener.key_values) | set(secondary_key_values_energy)):
            ...
            set(attr_powr.key_values) | set(secondary_key_values_power)
        ):
            # convert to the key specification
    ...
    def get_gwp(
        ...
        if key_dict not in self.dict_attributes["emission_gas"].field_maps.keys():
            valid_gwps = sf.format_print_list(self.configuration.valid_gwp)
            raise KeyError(
                f"Invalid GWP '{gwp}': defined global warming potentials are {valid_gwps}.",
            )
        ...
        valid_gasses = sf.format_print_list(
            self.dict_attributes["emission_gas"].key_values,
        )
        raise KeyError(f"Invalid gas '{gas}': defined gasses are {valid_gasses}.")

    def get_length_equivalent(
        self,
        length: str,
    ...
    def check_projection_input_df(
        ...
        if len(fields_dims_notime) > 0:
            ...
            if len(df_fields_dims_notime) > 1:
                raise ValueError(
                    f"Error in project: the input data frame contains multiple dimensions of analysis. The project method is restricted to a single dimension of analysis. The following dimensions were found:\n{df_fields_dims_notime}",
                )
            dict_dims = dict(
                zip(
                    fields_dims_notime,
        ...
        if (not set_times_project.issubset(set_times_defined)) and (
            not drop_invalid_time_periods
        ):
            sf.check_set_values(
                set_times_project,
                set_times_defined,
        ...
        if set_times_keep != set(range(min(set_times_keep), max(set_times_keep) + 1)):
            if not interpolate_missing_q:
                raise ValueError(
                    "Error in specified times: some time periods are missing and interpolate_missing_q = False. Modeling will not proceed. Set interpolate_missing_q = True to interpolate missing values.",
                )
        ...
        * Assumes that variable schema are unique for each model variable
        ...
        for var_int in variables_transfer:
            ...
            try:
                ...
                if stop_on_error:
                    raise RuntimeError(
                        f"Error in transfer_df_variables: get_optional_or_integrated_standard_variable returned {e}.",
                    )
    ...
    def _add_specified_total_fields_to_emission_total(
        self,
        df_in: pd.DataFrame,
        varlist: list,
    ) -> None:
        ...
        for subsec in dict_totals:
            array_totals = np.sum(dict_totals[subsec], axis=1)
            field_total = dict_fields[subsec]
            cur_emissions = (
                np.array(df_in[field_total]) if (field_total in df_in.columns) else 0
            )
            df_in[field_total] = cur_emissions + array_totals

    def add_subsector_emissions_aggregates(
        self,
        df_in: pd.DataFrame,
        ...
        for subsector in list_subsectors:
            ...
            for var in vars_subsec:
                ...
                if (var_type == "output") and gas:
                    total_emission_modvars_by_gas = (
                        self.dict_gas_to_total_emission_modvars.get(gas)
                    )
    ...
    def exchange_year_time_period(
        ...
        if direction in ["time_period_as_year"]:
            ...
            raise ValueError(
                f"Invalid direction '{direction}' in exchange_year_time_period: specify 'time_period_to_year' or 'year_to_time_period'.",
            )
    ...
    def array_to_df(
        ...
        if len(fields) != arr_in.shape[1]:
            ...
            raise ValueError(
                f"Array shape mismatch for fields {flds_print}: the array only has {arr_in.shape[1]} columns.",
            )

        return pd.DataFrame(arr_in * scalar_em * scalar_me, columns=fields)

    def assign_keys_from_attribute_fields(
        self,
        subsector: str,
        field_attribute: str,
        ...
        for val in all_field_values:
            ...
            for subsec_key in subsec_keys:
                for k in dict_assignment:
                    if k in subsec_key:
                        val_assigned = (
                            clean_schema(subsec_key) if clean_attr_key else subsec_key
                        )
                        dict_assigned.update({dict_assignment[k]: val_assigned})

            dict_out.update({dict_out_key: dict_assigned})
            dict_vals_unassigned.update(
                {
                    dict_out_key: list(
    ...
    def get_vars_by_assigned_class_from_akaf(
        self,
        dict_in: dict,
        var_class: str,
    ) -> list:
        ...
        return [
            x.get(var_class) for x in dict_in.values() if (x.get(var_class) is not None)
        ]

    def build_default_sampling_range_df(
        self,
    ) -> pd.DataFrame:
    ...
    def build_variable_dataframe_by_sector(
        self,
        ...
        for sector in sectors_build:
            ...
            for subsector in subsectors:
                ...
                vars_cur = sum(
                    [
                        self.dict_model_variables_to_variables.get(x)
                        for x in modvars_cur
                    ],
                    [],
                )
        ...
        if include_simplex_group_as_trajgroup:
            ...
            df_out[field_variable_trajectory_group] = df_out[
        ...
        if isinstance(df_trajgroup, pd.DataFrame):
            fields_sort_with_tg = fields_sort + [
                field_variable_trajectory_group,
            ]  # HEREHERE

            if set([field_variable, field_variable_trajectory_group]).issubset(
                ...
                if field_variable_trajectory_group in df_out.columns:
                    ...
                    df_out_keep = df_out[~df_out[field_variable].isin(vars_to_assign)]
                    df_out_overwrite = df_out[
                        df_out[field_variable].isin(vars_to_assign)
                    ].drop(
                        [field_variable_trajectory_group],
                        axis=1,
                    )
                    ...
                    (
                        df_out_keep[field_variable_trajectory_group].replace(
                            dict_to_reindex,
                            inplace=True,
                        )
                    )
                    ...
                    df_out_overwrite = pd.merge(
                        ...
                        df_trajgroup[
                            [
                                x
                                for x in fields_sort_with_tg
                                if x in df_trajgroup.columns
                            ]
                        ].dropna(),
                        how="left",
                    )
        ...
        df_out = df_out.sort_values(by=fields_sort).reset_index(drop=True)

        return df_out

    def build_vars_basic(
        self,
    ...
    def build_vars_outer(
        ...
        for var in vars_to_loop:
            ...
            if (cat_i not in var_schema) or (cat_j not in var_schema):
                raise ValueError(
                    f"Error in {var} variable schema: one of the outer categories '{cat_i}' or '{cat_j}' was not found. Check the attribute file.",
                )

            for catval_i in dict_vars_to_cats[var]:
                for catval_j in dict_vars_to_cats[var]:
                    vars_out.append(
                        var_schema.replace(cat_i, catval_i).replace(cat_j, catval_j),
                    )

        return vars_out

    def build_target_varlist_from_source_varcats(
        self,
        modvar_source: str,
        modvar_target: str,
    ):
    ...
    def build_varlist(
        ...
        category = (
            attr_subsec.field_maps.get(f"{attr_subsec.key}_to_primary_category")
            .get(abv_subsec)
            .replace("`", "")
        )
        ...
        dict_vrp_vvs_cats, dict_vrp_vvs_cats_outer = (
            self.get_partial_category_dictionaries(
                subsector,
                category_ij_tuple,
    ...
    def check_category_restrictions(
        self,
        ...
        if categories_to_restrict_to is not None:
            ...
            valid_cats = [
                x for x in attribute_table.key_values if x in categories_to_restrict_to
            ]
            invalid_cats = sorted(
                [
                    x
    ...
    def clean_partial_category_dictionary(
        self,
        dict_in: dict,
        ...
        for k in dict_in:
            if dict_in[k].lower().replace(" ", "") == "none":
                ...
                if len(missing_vals) > 0:
                    ...
                    warnings.warn(
                        f"clean_partial_category_dictionary: Invalid categories values {missing_vals} dropped when cleaning the dictionary. Category values not found.",
                    )
        return dict_in

    def format_category_for_direct(
        self,
        category_to_replace: str,
        appendstr_i="-I",
        appendstr_j="-J",
    ...
    def get_input_output_fields(
        ...
        if build_df_q:
            vars_in = (
                pd.DataFrame({"subsector": subsectors_in, "variable": vars_in})
                .sort_values(by=["subsector", "variable"])
                .reset_index(drop=True)
            )
            vars_out = (
                pd.DataFrame({"subsector": subsectors_out, "variable": vars_out})
                .sort_values(by=["subsector", "variable"])
                .reset_index(drop=True)
            )
    ...
    def get_multivariables_with_bounded_sum_by_category(
        self,
        df_in: pd.DataFrame,
        modvars: list,
        ...
        for modvar in modvars:
            if modvar not in self.dict_model_variables_to_variables.keys():
                raise ValueError(
                    f"Invalid variable specified in get_standard_variables: variable '{modvar}' not found.",
                )
            ...
            if init_q:
                ...
                raise ValueError(
                    "Error in get_multivariables_with_bounded_sum_by_category: variables must be from the same subsector.",
                )
    ...
    def get_optional_or_integrated_standard_variable(
        self,
        df_in: pd.DataFrame,
        var_integrated: str,
        var_optional: str,
    ...
    def get_partial_category_dictionaries(
        ...
        valid_cats = self.check_category_restrictions(
            restrict_to_category_values,
            self.dict_attributes[
                self.get_subsector_attribute(subsector, "pycategory_primary")
            ],
        )
        ...
        if key_attribute is not None:
            dict_vr_vvs_cats_ud, dict_vr_vvs_cats_outer = (
                self.separate_varreq_dict_for_outer(
                    subsector,
                    key_type,
    ...
    def get_sector_list_from_projection_input(
        self,
        ...
        list_out = [
            dict_map.get(x, x)
            for x in list_out
            if dict_map.get(x, x) in valid_sectors_project
        ]

        return list_out

    def get_simplex_group(
        self,
        variable: str,
    ) -> Union[int, None]:
    ...
    def get_simplex_group_specification(
        self,
        ...
        category_specs = set(
            [(self.get_variable_categories(x) is None) for x in modvars_to_check],
        )
        ...
        if len(modvars_to_check) == 1:
            ...
            for cat in cats:
                ...
                for modvar in modvars_to_check:
                    ...
                    (fields_group.append(field) if field is not None else None)
    ...
    def get_standard_variables(
        ...
        if return_type == "data_frame":
            flds = [flds] if (type(flds) not in [list, np.ndarray]) else flds
    ...
    def get_subsector_emission_total_field(
        self,
        subsector: str,
    ...
    def get_subsector_variables(
        ...
        category = (
            self.dict_attributes[self.table_name_attr_subsector]
            .field_maps["abbreviation_subsector_to_primary_category"][
                self.get_subsector_attribute(subsector, "abv_subsector")
            ]
            .replace("`", "")
        )
        category_ij_tuple = self.format_category_for_direct(category, "-I", "-J")
            ...
            for x in dicts:
                ...
                dict_var_type.update(
                    dict(
                        zip(
                            l_vars,
                            [key_type.replace("key_varreqs_", "") for x in l_vars],
                            strict=False,
                        ),
                    ),
                )
    ...
    def get_variable_attribute(
        ...
        if variable not in self.all_model_variables:
            raise ValueError(
                f"Invalid model variable '{variable}' found in get_variable_characteristic.",
            )
    ...
    def get_variable_characteristic(
        self,
        variable: str,
        characteristic: str,
    ) -> str:
    ...
    def get_variable_from_category(
        self,
        subsector: str,
        category: str,
        var_type: str = "all",
        ...
        if var_type not in ["all", "partial"]:
            raise ValueError(
                f"Invalid var_type '{var_type}' in get_variable_from_category: valid types are 'all', 'partial'",
            )
    ...
    def get_variable_unit_conversion_factor(
        self,
        var_to_convert: str,
        var_to_match: str,
        units: str,
        ...
        # check values
        if units not in dict_valid_units:
            str_valid_units = sf.format_print_list(
                sorted(list(dict_valid_units.keys())),
            )
            raise ValueError(
                f"Invalid units '{units}' specified in get_variable_conversion_factor: valid values are {str_valid_units}",
            )
    ...
    def get_variables_by_sector(
        ...
        if return_var_type == "input":
            return vars_input
        if return_var_type == "output":
            return vars_output
        if return_var_type == "both":
            vars_both = sorted(vars_input + vars_output)
            return vars_both
        raise ValueError(
            f"Invalid return_var_type specification '{return_var_type}' in get_variables_by_sector: valid values are 'input', 'output', and 'both'.",
        )

    def get_variables_from_attribute(
        self,
        subsec: str,
        dict_attributes: str,
    ) -> Union[List[str], None]:
        ...
        for key in ["key_varreqs_all", "key_varreqs_partial"]:
            ...
            vars_out = vars_match if vars_out is None else (vars_out + vars_match)

        return vars_out

    def get_variable_to_simplex_group_dictionary(
        self,
        ...
        for sort_class in all_sort_class:
            ...
            dict_field_to_group_ind = dict(
                (field, i) for i, g in enumerate(groups) for field in g.keys()
            )
    ...
    def instantiate_blank_modvar_df_by_categories(
        self,
        modvar: str,
        n: int,
    ...
    def separate_varreq_dict_for_outer(
        ...
        if key_attribute is not None:
            dict_vr_vvs = (
                self.dict_varreqs[self.get_subsector_attribute(subsector, key_type)]
                .field_maps[f"variable_to_{field_to_split_on}"]
                .copy()
            )
            dict_vr_vtf = (
                self.dict_varreqs[self.get_subsector_attribute(subsector, key_type)]
                .field_maps[f"variable_to_{target_field}"]
                .copy()
            )
            ...
            if variable_type is not None:
                if variable is not None:
                    warnings.warn(
                        "variable and variable_type both specified in separate_varreq_dict_for_outer: the variable assignment is higher priority, and variable_type will be ignored.",
                    )
                else:
                    dict_var_types = self.dict_varreqs[
                        self.get_subsector_attribute(subsector, key_type)
                    ].field_maps["variable_to_variable_type"]
                    drop_vars = [
                        x
                        for x in dict_var_types.keys()
                        if dict_var_types[x].lower() != variable_type.lower()
                    ]
            ...
            try:
                vars_outer = [
                    x
                    for x in dict_vr_vtf.keys()
                    if (category_outer_tuple[0] in dict_vr_vvs[x])
                    and (category_outer_tuple[1] in dict_vr_vvs[x])
                ]
                ...
                raise ValueError(
                    f"Invalid attribute table designations found in subsector '{subsector}': check the field '{target_field}'.",
                )
    ...
    def swap_array_categories(
        ...
        if len(set_drops_source) > 0:
            ...
            warnings.warn(
                f"Source values {vals_dropped_source} dropped in swap_array_categories (either not well-defined categories or there was no associated target category).",
            )
        ...
        if len(set_drops_target) > 0:
            ...
            warnings.warn(
                f"target values {vals_dropped_target} dropped in swap_array_categories (either not well-defined categories or there was no associated target category).",
            )

        # build dictionary and set up the new categories
        dict_swap = dict(zip(vec_source, vec_target, strict=False))
    ...
    def get_mutex_cats_for_internal_variable(
        self,
        subsector: str,
        variable: str,
        ...
        sum_cvs = list(
            df_tmp[df_tmp[attribute_sum_specification_field].isin([1])][pycat_primary],
        )
        ...
        if return_type == "fields":
            return fields_sum
        if return_type == "category_values":
            return sum_cvs
        raise ValueError(
            f"Invalid return_type '{return_type}'. Please specify 'fields' or 'category_values'.",
        )
    ...
    def get_simple_input_to_output_emission_arrays(
        ...
        for var in dict_vars:
            subsector_var, driver_unit_type, scale_factor = (
                self.dict_model_variable_to_subsector[var]
            )
            if subsector_driver != subsector_driver:
                warnings.warn(
                    f"In get_simple_input_to_output_emission_arrays, driver variable '{variable_driver}' and emission variable '{var}' are in different sectors. This instance will be skipped.",
                )
            else:
    ...
    def manage_internal_variable_to_df(
        self,
        df_in: pd.DataFrame,
        subsector: str,
        ...
        field_check = self.build_varlist(subsector, variable_subsec=internal_variable)[
            0
        ]
        ...
        if action not in valid_actions:
            ...
            raise ValueError(
                f"Invalid actoion '{action}': valid actions are {str_valid}.",
            )
        if action == "check":
        ...
        if action == "remove":
            ...
            if field_check not in df_in.columns:
                ...
                fields_sum = self.get_mutex_cats_for_internal_variable(
                    subsector,
                    component_variable,
                    attribute_sum_specification_field,
                    "fields",
                )
