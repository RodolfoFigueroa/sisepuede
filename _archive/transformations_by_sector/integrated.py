...
class TransformersIntegrated:
    ...
    def _initialize_attributes(
        ...
        * self.time_periods (support_classes.TimePeriods object)
        ...
        if error_q:
            raise RuntimeError(
                "Error: invalid specification of model_attributes in TransformersIPPU",
            )
    ...
    def _initialize_base_input_database(
        self,
        regions: Union[List[str], None] = None,
        use_demo_template_on_missing: bool = True,
    ) -> None:
    ...
    def _initialize_baseline_inputs(
        ...
        NOTE: Additionally, to preserve consistency when calling transformations
            from the TransformersIntegrated object, this overwrites baseline
    ...
    def _initialize_models(
        ...
        * self.model_socioeconomic
        ...
        self.model_circular_economy = (
            self.transformations_circular_economy.model_circecon
        )
    ...
    def _initialize_parameters(
        ...
        """
        dict_config = dict_config = self.config if dict_config is None else dict_config
    ...
    def _initialize_sectoral_transformations(
        ...
        * NOTE: this is set in both `self.transformation_af_baseline()` and
        ...
        """
        dict_config = self.config if dict_config is None else dict_config
        # initialize all transformations with df_input
        # then use those functions to set baseline_inputs

        self.transformations_afolu = dta.TransformersAFOLU(
            self.model_attributes,
            dict_config,
            baseline_with_plur=baseline_with_plur,
            df_input=df_input,
        ...
        self.transformations_circular_economy = dtc.TransformersCircularEconomy(
            self.model_attributes,
            dict_config,
            df_input=df_input,
            field_region=self.key_region,
        ...
        self.transformations_energy = dte.TransformersEnergy(
            self.model_attributes,
            dict_config,
        ...
        self.transformations_ippu = dti.TransformersIPPU(
            self.model_attributes,
            dict_config,
            df_input=df_input,
            field_region=self.key_region,
    ...
    def _initialize_transformations(
        ...
        function_list = (
            self.transformations_circular_economy.ce_all.function_list.copy()
        )
        ...
        function_list_plur += self.transformations_afolu.af_all_with_partial_reallocation.function_list.copy()
        ...
        function_list_india_ccdr = [
            x
            for x in function_list_india_ccdr
        ...
        if (
            self.transformations_afolu.transformation_lndu_integrated_transitions
            in function_list_plur_no_silvopasture
        ):
        ...
        function_list_plur_no_deforestation_stoppage += self.transformations_afolu.af_all_with_deforestation_and_partial_reallocation.function_list.copy()
    ...
    def _try_template_init_from_demo(
        self,
    ...
    def build_strategies_to_templates(
        self,
        ...
        **kwargs,
        ...
        for r, df in df_out_grouped:
            ...
            dict_cur = dict_write.get(r) if return_q else dict_sectors.copy()
            ...
            for i, strat in enumerate(strategies):
                ...
                fields_drop = [
                    x for x in df_cur.columns if x not in fields_var_all + fields_sort
                ]
                df_cur = (
                    df_cur.drop(fields_drop, axis=1)
            ...
            if isinstance(dict_exogenous_grouped, dict):
                ...
                if df_cur_grouped is not None:
                    ...
                    for strat, df_exog in df_cur_grouped:
                        ...
                        fields_drop = [
                            x
                            for x in df_cur.columns
                            if x not in fields_var_all + fields_sort
                        ]
                        df_cur = (
                            df_cur.drop(fields_drop, axis=1)
                            .sort_values(by=fields_sort)
                        ...
                        dict_cur = (
                            self.build_templates_dictionary_from_current_transformation(
                                df_cur,
                                attr_sector,
                                dict_cur,
                                **kwargs,
    ...
    def build_templates_dictionary_from_current_transformation(
        self,
        df_cur: pd.DataFrame,
    ...
    def check_exogenous_strategies(
        self,
    ...
    def transformation_pflo_baseline(
        self,
        df_input: pd.DataFrame,
        strat: Union[int, None] = None,
    ) -> pd.DataFrame:
    ...
    def transformation_pflo_healthier_diets(
        self,
        df_input: Union[pd.DataFrame, None] = None,
    ...
    def transformation_pflo_industrial_ccs(
        self,
        df_input: Union[pd.DataFrame, None] = None,
        strat: Union[int, None] = None,
        vec_implementation_ramp: Union[np.ndarray, None] = None,
