...
def transformation_agrc_improve_crop_residue_management(
    ...
    **kwargs,
    ...
    """
    model_afolu = mafl.AFOLU(model_attributes) if model_afolu is None else model_afolu
...
def transformation_agrc_increase_no_till(
    ...
    **kwargs,
    ...
    magnitude = (
        ...
        if sf.isnumber(magnitude)
        else dict((k, float(sf.vec_bounds(v, bounds))) for k, v in magnitude.items())
    )
...
def transformation_frst_reduce_deforestation(
    ...
    **kwargs,
    ...
    magnitude = (
        float(sf.vec_bounds(magnitude, (0.0, 1.0))) if sf.isnumber(magnitude) else None
    )
    ...
    dict_magnitude.update(
        dict(
            ...
            if (x != y) and (x != cat_lndu_frst_primary)
        ),
    )
    ...
    (df_out.drop([field_region], axis=1, inplace=True) if use_fake_region else None)
    ...
    **kwargs,
    ...
    cats_ir = (  # [cat_crop, cat_grsl]
        [
            x
            for x in attr_lndu.key_values
            if x in cats_inflow_restriction and x != cat_fsts
        ]
...
def transformation_support_lndu_check_ltct_magnitude_dictionary(
    ...
    for cat in cats_all:
        ...
        categories_scalar_reference = (
            [x for x in categories_scalar_reference if x in cats_all and x != cat]
        ...
        tp_baseline = (
            max(time_periods.all_time_periods)
            if (tp_baseline not in time_periods.all_time_periods)
            else tp_baseline
        )
...
def transformation_support_lndu_check_pasture_magnitude(
    ...
    area_grassland_new = (
        area_grassland_b1 * max_change_allocated_to_pasture_frac_adjustment
    )
    area_grassland_new += area_grassland_b0 * (
        1 - max_change_allocated_to_pasture_frac_adjustment
    )
...
def transformation_support_lndu_get_adjusted_fractions_from_transition_w_natural_grassland(
    ...
    vec_lndu_prevalence_natural_grassland_base = (
        vec_lndu_prevalence_grass_base - vec_lndu_prevalence_pasture_base
    )

    # calculate carrying target for new pasture
    vec_lndu_carrying_target_base = (
        vec_lndu_prevalence_pasture_base
        * vec_lvst_scalar_carrying_capacity_no_intervention
    )

    # now, get the area associated with pasture now and generate the scalar
    vec_lndu_prevalence_grass_intervention = (
        arr_land_use_prevalence_out_with_intervention[:, model_afolu.ind_lndu_grass]
    )
    vec_lndu_prevalence_pasture_intervention = sf.vec_bounds(
        vec_lndu_prevalence_grass_intervention
        - vec_lndu_prevalence_natural_grassland_base,
        (min_frac_grassland_pasture, 1.0),
    )
    vec_lndu_carrying_capacity_new = (
        vec_lndu_carrying_target_base / vec_lndu_prevalence_pasture_intervention
    )

    # get new pasture fraction
    vec_lndu_pasture_frac_new = np.nan_to_num(
        vec_lndu_prevalence_pasture_intervention
        / vec_lndu_prevalence_grass_intervention,
        nan=0.0,
        posinf=0.0,
    )
...
def transformation_support_lndu_transition_to_category_targets_single_region(
    ...
    **kwargs,
    ...
    arr_land_use_prevalence_out_with_intervention[0:ind_last_zero] = (
        arr_land_use  # land use up until final time period
    )
    ...
    for ind_row, i in enumerate(inds_iter):
        ...
        for j, z in enumerate(inds_to_modify):
            ...
            dict_adj.update(
                dict(((ind, z), s) for ind, s in enumerate(scalar_lndu_cur)),
            )
...
def transformation_lsmm_improve_manure_management(
    ...
    **kwargs,
    ...
    dict_lsmm_magnitude = dict(
        ...
        if k in categories_lsmm and sf.isnumber(v)
    )
...
def transformation_soil_reduce_excess_fertilizer(
    ...
    **kwargs,
    ...
    for key, modvar in dict_key_to_modvar.items():
        ...
        mag = float(sf.vec_bounds(1 - mag, (0, 1))) if mag is not None else None
